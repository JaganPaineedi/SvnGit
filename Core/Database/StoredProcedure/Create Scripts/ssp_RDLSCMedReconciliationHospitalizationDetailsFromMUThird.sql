 IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLSCMedReconciliationHospitalizationDetailsFromMUThird]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[ssp_RDLSCMedReconciliationHospitalizationDetailsFromMUThird]
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[ssp_RDLSCMedReconciliationHospitalizationDetailsFromMUThird] (      
 @StaffId INT      
 ,@StartDate DATETIME      
 ,@EndDate DATETIME      
 ,@MeasureType INT      
  ...
  FROM (      
   ...
   WHERE CI.STATUS <> 4981      
    ...
    AND isnull(BA.RecordDeleted, 'N') = 'N'      
     AND ((@MeaningfulUseStageLevel= 8768 and @Tin ='NA' )                   
