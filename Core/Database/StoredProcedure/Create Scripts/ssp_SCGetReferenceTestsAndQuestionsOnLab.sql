/****** Object:  StoredProcedure [dbo].[ssp_SCGetReferenceTestsAndQuestionsOnLab]    Script Date: 10/29/2014 15:03:31 ******/
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[ssp_SCGetReferenceTestsAndQuestionsOnLab]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[ssp_SCGetReferenceTestsAndQuestionsOnLab]
GO
...
CREATE PROCEDURE [dbo].[ssp_SCGetReferenceTestsAndQuestionsOnLab]
@LabId NVARCHAR(100),
...
/*      
 ...
 Pradeep		01/22/2016				TimeOut updated to -1 to avoid timeout. 
...
BEGIN
	BEGIN TRY		
		...
		BEGIN
			...
			SELECT  @ReferenceLabs = CONVERT(XML,[dbo].[GetReferenceTestsAndQuestionsOnLab](@LabSoftServiceUrl,@TimeOut,@ServiceKey,@ServiceSecret,@LabId,@LabName));
			--SELECT @ReferenceLabs
	...
	BEGIN CATCH
		...
		SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_SCGetReferenceTestsAndQuestionsOnLab') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())
