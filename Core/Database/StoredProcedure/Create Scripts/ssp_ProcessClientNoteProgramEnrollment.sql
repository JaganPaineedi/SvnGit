...
/****** Object:  StoredProcedure [dbo].[ssp_ProcessClientNoteProgramEnrollment]    Script Date: 07/26/2018 16:23:57 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_ProcessClientNoteProgramEnrollment]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_ProcessClientNoteProgramEnrollment]
GO
...
CREATE PROCEDURE [dbo].ssp_ProcessClientNoteProgramEnrollment (@ClientProgramId INT)
AS
/******************************************************************************                                  
**  File: ssp_ProcessClientNoteProgramEnrollment.sql                
**  Name: ssp_ProcessClientNoteProgramEnrollment           
**  Desc:                 
**                                  
...
**  Date:			Author:    Description: 
	Aug 06 2018		Ravi		Create Client Flags when Client is Enrolled a program  
...
BEGIN
	BEGIN TRY
		...
		WHERE CAST(TP.StartDate AS DATE) <= CAST(CP.EnrolledDate AS DATE)
			AND (TP.EndDate IS NULL OR (CAST(TP.EndDate AS DATE) >= CAST(CP.EnrolledDate AS DATE)))
			...
			AND (TPF.FlagAssignment = 'M'
				...
				WHERE CPs.ClientProgramId = @ClientProgramId
				    AND (CTTM.StartDate IS NULL OR (CAST(CTTM.StartDate AS DATE) <= CAST(CPs.EnrolledDate AS DATE))) 
					AND (CTTM.EndDate IS NULL OR (CAST(CTTM.EndDate AS DATE) >= CAST(CPs.EnrolledDate AS DATE)))
			...
			AND GC3.Code='On Program Enrollment'
	...
	BEGIN CATCH
		...
		SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_ProcessClientNoteProgramEnrollment') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())
