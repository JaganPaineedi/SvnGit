/****** Object:  Trigger [TriggerInsertUpdate_ServiceDropdownConfigurations]    Script Date: 07/07/2016 18:37:01 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[TriggerInsertUpdate_ServiceDropdownConfigurations]'))
DROP TRIGGER [dbo].[TriggerInsertUpdate_ServiceDropdownConfigurations]
GO

/****** Object:  Trigger [dbo].[TriggerInsertUpdate_ServiceDropdownConfigurations]    Script Date: 07/07/2016 18:37:01 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[TriggerInsertUpdate_ServiceDropdownConfigurations] ON [dbo].[ServiceDropdownConfigurations]
  FOR INSERT, UPDATE
/*********************************************************************  
...
WHERE object_id = object_id('ServiceDropdownConfigurations')
...
DECLARE @TableId int
SET @TableId = (SELECT AuditLogTableId FROM dbo.AuditLogTables WHERE TableName = 'ServiceDropdownConfigurations' AND isnull(RecordDeleted, 'N') = 'N')
...
DECLARE @Error varchar(8000)                                                                                                                                 
   ...
   + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'TriggerInsertUpdate_ServiceDropdownConfigurations')                     
