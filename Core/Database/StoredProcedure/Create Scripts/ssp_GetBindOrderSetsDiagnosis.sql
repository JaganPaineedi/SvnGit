IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_GetBindOrderSetsDiagnosis]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_GetBindOrderSetsDiagnosis]
GO 
  
 CREATE PROCEDURE [dbo].[ssp_GetBindOrderSetsDiagnosis]          
 @ClientId INT,
 @OrderSetId INT,
 @OrderIdList VARCHAR(MAX)    
AS           
  /**************************************************************              
  Created By   : Pabitra [ssp_GetBindOrderSetsDiagnosis] 4,3,'26'         
  ...
  SET @LatestICD10DocumentVersionID = (      
    ...
    ORDER BY a.EffectiveDate DESC      
     ,a.ModifiedDate DESC      
	    ...
	    INNER JOIN Orders O ON O.OrderId=OD.OrderId AND O.OrderId IN(SELECT   O.OrderId FROM OrderSets  OS   
        ...
        INNER JOIN Orders O ON O.OrderId=OSA.OrderId AND ISNULL(O.RecordDeleted,'N')='N' AND O.OrderId NOT IN (SELECT token
        FROM dbo.SplitString(@OrderIdList, ',')) AND O.EMNoteOrder='Y' WHERE  OS.OrderSetId=@OrderSetId
         ) 
	    LEFT JOIN DiagnosisICD10ICD9Mapping DICD ON DICD.ICD10CodeId = OD.ICD10CodeId
	    WHERE ISNULL(OD.RecordDeleted, 'N') <> 'Y'  
	    AND (OD.ICD10CodeId NOT IN (SELECT DSMCode FROM #DiagnosisTemp))
