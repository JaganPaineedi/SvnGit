...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLSCImagingHospitalization]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_RDLSCImagingHospitalization]
GO
...
CREATE PROCEDURE [dbo].[ssp_RDLSCImagingHospitalization] (      
 @StaffId INT      
 ,@StartDate DATETIME      
 ,@EndDate DATETIME      
 ,@MeasureType INT      
 ...
 /********************************************************************************          
-- Stored Procedure: dbo.ssp_RDLSCLabImagingHospitalization            
--          
...
BEGIN      
 ...
 BEGIN CATCH      
  ...
  SET @error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_RDLSCImagingHospitalization') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' +    
