 IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLSCViewDownloadHospitalizationDetailsFromMUThird]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[ssp_RDLSCViewDownloadHospitalizationDetailsFromMUThird]
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[ssp_RDLSCViewDownloadHospitalizationDetailsFromMUThird] (    
 @StaffId INT    
 ,@StartDate DATETIME    
 ,@EndDate DATETIME    
 ,@MeasureType INT    
 ...
 /********************************************************************************          
-- Stored Procedure: dbo.[ssp_RDLSCViewDownloadHospitalizationDetailsFromMUThird]            
--          
...
BEGIN    
 ...
 BEGIN CATCH    
  ...
  SET @error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), '[ssp_RDLSCViewDownloadHospitalizationDetailsFromMUThird]') + '*****' + CONVERT(VARCHAR, ERROR_LINE
()) + '*****  
