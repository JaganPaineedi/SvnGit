IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLSCPatientEducationHospitalization]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_RDLSCPatientEducationHospitalization]
GO
...
CREATE PROCEDURE [dbo].[ssp_RDLSCPatientEducationHospitalization] (    
 @StaffId INT    
 ,@StartDate DATETIME    
 ,@EndDate DATETIME    
 ,@MeasureType INT    
 ...
 /********************************************************************************        
-- Stored Procedure: dbo.ssp_RDLSCPatientEducationHospitalization          
--        
-- Copyright: Streamline Healthcate Solutions     
--        
-- Updates:                                                               
-- Date    Author   Purpose        
-- 04-Nov-2014  Revathi  What:Patient Hospitalization.              
...
BEGIN    
 BEGIN TRY   
  ...
  SET R.Denominator = (    
    ...
    INNER JOIN Clients C1 ON C1.ClientId = CI.ClientId  AND isnull(C1.RecordDeleted, 'N') = 'N'    
