 IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'ssp_SCListPageLaboratories')
    BEGIN
        DROP  Procedure  ssp_SCListPageLaboratories 
    END

GO

CREATE PROCEDURE [dbo].[ssp_SCListPageLaboratories]                                                                           
...
AS                                                                  
/****************************************************************************/                                                      
/* Stored Procedure: [ssp_SCListPageLaboratories]                           */                                             
/* Copyright: 2011 Streamlin Healthcare Solutions                           */           
/* Author: Maninder                  */                                                     
/* Creation Date:  Dec 14,2011                                              */                                                      
/* Purpose: Get Data for list page in My office tab for LabResults          */                                                     
                                                
/* Output Parameters:None                                                   */                                                      
/* Return:                                                                  */                                                      
/* Calls:                                                                   */          
/* Called From: Laboratories.GetLaboratoriesList() from Dataservice         */                                                      
/* Data Modifications:                                                      */                                                      
/*                                                                          */  
...
/*  31/08/2016   Ravichandra		Removed the physical table ListPageSCLaboratoryResults from SP
...
INSERT INTO #ResultSet(                                                                  
		 ...
		 GC.GlobalCodeId,
		 LR.DateReceived,  
		 isnull(LR.ClientLastName,'')+', '+ISNULL(LR.ClientFirstName,'') as ClientName, 
		 GC.CodeName,  
		Isnull(LR.LaboratoryName,'') AS LaboratoryName, isnull(LR.LaboratoryAddress,'')+', '+ISNULL(LR.LaboratoryCity,'')+', '+ISNULL(LR.LaboratoryState,'')+ ' '+ISNULL(LR.LaboratoryZip,'') as  LaboratoryAddress,  
		LR.ClientOrderId  
	...
	AND (LR.ClientId=@ClientIdFilter OR ISNULL(@ClientIdFilter,0)=0)  
...
SET @Error= Convert(varchar,ERROR_NUMBER()) + '*****' + Convert(varchar(4000),ERROR_MESSAGE())         
    + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'ssp_SCListPageLaboratories')                                                                                                                       
