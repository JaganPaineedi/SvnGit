/****** Object:  StoredProcedure [dbo].[ssp_GetLaboratoryTests]     ******/
IF EXISTS (
		SELECT 1
		FROM INFORMATION_SCHEMA.ROUTINES
		WHERE SPECIFIC_SCHEMA = 'dbo'
			AND SPECIFIC_NAME = 'ssp_GetLaboratoryTests'
		)
	DROP PROCEDURE [dbo].[ssp_GetLaboratoryTests]
GO
...
CREATE PROCEDURE [dbo].[ssp_GetLaboratoryTests] @ClientId INT = NULL
...
BEGIN
			...
			BEGIN
				...
				SELECT @JsonResult = dbo.smsf_FlattenedJSON((
							SELECT DISTINCT c.ClientId         
								,Co.OrderType AS BasedOn --Reference  Reference(CarePlan|ImmunizationRecommendation|MedicationRequest|NutritionOrder|ProcedureRequest|ReferralRequest)          
								...
								,Isnull('LOINC:' + ob.LOINCCode + ' ' + o.ObservationName, o.ObservationName) AS Specimen --Reference                     
							...
							LEFT JOIN Observations ob ON ob.ObservationId = coo.ObservationId
							...
							WHERE c.ClientId = @ClientId
								...
								AND (
									Co.OrderType = 'Radiology' --6482        
									OR Co.OrderType = 'Labs' --6481        
	...
	BEGIN CATCH
		...
		SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_GetLaboratoryTests') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())
