IF EXISTS (SELECT * FROM sysobjects WHERE type = 'FN' AND name = 'ssf_ReplacePhraseText')
	BEGIN
		DROP  Function  ssf_ReplacePhraseText
	END

GO
 CREATE FUNCTION dbo.ssf_ReplacePhraseText
(
...
AS
 BEGIN								       
  ...
  Set @KeyPhraseTextTags=Replace(@KeyPhraseTextTags,'''','&#39')
