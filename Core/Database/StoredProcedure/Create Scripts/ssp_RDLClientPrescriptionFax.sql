IF EXISTS ( SELECT	*
			FROM	sys.objects
			WHERE	object_id = OBJECT_ID(N'[dbo].[ssp_RDLClientPrescriptionFax]')
					AND type IN ( N'P', N'PC' ) ) 
	DROP PROCEDURE [dbo].[ssp_RDLClientPrescriptionFax]
GO

/****** Object:  StoredProcedure [ssp_RDLClientPrescriptionFax]    Script Date: 1/2/2013 2:12:48 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [ssp_RDLClientPrescriptionFax]
...
/*===========================================================================================================
Procedure Name:ssp_RDLClientPrescriptionFax
Purpose : Display  medication details on the Fax Preview RDL
Modifications:
*Name*         *Date*         *Purpose*
	...
	Pranay -04/April/2017- Pranay -04/April/2017- Added condition  @Preview 2. If Preview 'Y' NADEA number is selected from the Preview tables Else from the Original tables
    ...
    Vithtobha		- 28/July/2017	- Merging changes of praorane - 3/24/2016 - Changed the organization name to display Northstar Center when the prescribing location starts with NS or Northstar. Riverwood - Support 959.
    Vithtobha		- 28/July/2017	- Merging changes of Sanjay-:-18-04-20176- I changed address column to address display column from location table Why: Key Point - Support Go Live #166
    ...
    Anto		    - 24/Aug/2017	- Merging changes of Wasif (Diagnosis fix) from valley environment to display ICD10Codeid in Fax RDL.	
    Anto		    - 19/Sep/2017	- Casting ICD10CodeId into varchar which fails in some scenario. Newaygo - Support: #679												
    Anto		    - 23/Sep/2017   - Modified the logic to display Chart copy on the RDL while faxing an order. KCMHSAS - Support #481
    Bernardin      - 18/Oct/2017	- What: Added record deleted condition to avoid selecting NA DEA No in rdl. Why: Thresholds-Support#1063
	Nandita			- 8/NOV/2017	- Added logic to check if atleast one phone is present for a client and not just 'Home' type	
    PranayB         -06/DEC/2017    -Added @AddressType w.r.t CEI-SupportGoLive Task#854
    PranayB        -12/DEC/2017    -Added ca.AddressType OrderBy w.r.t CEI -support Task#854
   PranayB         -05/Jan/2018    -Added @PhoneNumberType w.r.t KCMHSAS - Support Task	#928 - Rx.
   ...
   pranayB         -19/Jan/2018    -Added Case statement for DxPurpose w.r.t Harbor Task#23
    ...
    PranayB         -11/July/2018          Added Addtional drugs to NADEA list w.r.t EPCS 75
	Vithobha		-29/Oct/2018	- Added OriginalDAW to get Dispensed as Written Checkbox value. Because to show the Electronic signature based on this condition. Comprehensive-Support Go Live: #58
===================================================================================================================*/
AS 
...
--check for c2 meds for signature options
	...
	CREATE TABLE #ClientScriptInstructions
		(
		  ...
		  OriginalDAW VARCHAR(1) --Vithobha		-29/Oct/2018
		)
...
-- Get Client Alergy List
	...
	--get Client Info
			INSERT	INTO #ScriptHeader
					...
					SELECT	( SELECT	OrganizationName
							...
							c.DOB AS ClientDOB,
							REPLACE(ISNULL(l.AddressDisplay,''),CHAR(13)+CHAR(10),' ') AS LocationAddress,--Modified by Sanjay
   ...
   -- get the deacodes for all ClientMedicationScriptDrugs
	...
	--Get Signature Info
			INSERT	INTO #ScriptSignatures
					...
					FROM	ClientMedicationScripts AS cms
							...
							LEFT JOIN StaffLicenseDegrees as sldForNADEA ON sldForNADEA.StaffLicenseDegreeId=cms.StaffLicenseDegreeId  
	...
	--Pharmacy Text Change
                            ...
                            ,cm.DAW AS OriginalDAW --Vithobha		-29/Oct/2018
					FROM	ClientMedicationScripts AS cms
							JOIN ClientMedicationScriptDrugs AS cmsd ON cmsd.ClientMedicationScriptId = CMS.ClientMedicationScriptId
															  AND ISNULL(cmsd.RecordDeleted,
															  'N') <> 'Y'
	...
	--Get Signature Info
			INSERT	INTO #ScriptSignatures
					...
					FROM	ClientMedicationScriptsPreview AS cms
							...
							JOIN Staff AS v ON v.UserCode = cms.CreatedBy
											   AND ISNULL(v.RecordDeleted, 'N') <> 'Y'
							JOIN dbo.StaffLicenseDegrees license ON license.StaffId = v.StaffId
							JOIN MDMedicationNames AS MedName ON MedName.MedicationNameId = cm.MedicationNameId
												AND ISNULL(MedName.RecordDeleted,
														   'N') <> 'Y'
							LEFT JOIN globalCodes AS vDeg ON vDeg.GlobalCodeId = v.Degree
	...
	BEGIN
		INSERT	INTO #ClientScriptInstructionResults
				( ClientMedicationScriptId,
				  ...
				  PotencyUnitCodeDescription,
				  DxPurpose,
				  OriginalDAW --Vithobha		-29/Oct/2018
                )
--Non Multiples, Non Titrations
	...
	--change special instructions to include comments if specified
						...
						csi.PotencyUnitCodeDescription
						,csi.DxPurpose,
						csi.OriginalDAW --Vithobha		-29/Oct/2018
				FROM	#ClientScriptInstructions csi
						JOIN #QuantitySummary qs ON qs.PON = csi.PON
...
ORDER BY			b.MedicationName,
		...
		BEGIN
		  ...
		  END
				...
				SELECT	CASE WHEN h.LocationName LIKE 'NS%' THEN 'Northstar Center'
					WHEN h.LocationName LIKE 'Northstar%' THEN 'Northstar Center'
						ELSE h.OrganizationName
				END OrganizationName,  -- 3/24/2016 praorane
				...
				,m.DxPurpose 
                ,m.OriginalDAW --Vithobha		-29/Oct/2018
		FROM	#ScriptHeader h
