...
/****** Object:  StoredProcedure [dbo].[ssp_CCRMessage]    Script Date: 06/09/2015 02:35:16 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_CCRMessage]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_CCRMessage]
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[ssp_CCRMessage]
    @ClientId BIGINT ,
    @StaffId BIGINT
AS 
    BEGIN
        ...
        SELECT  'Staff.' + CAST(@StaffId AS VARCHAR(100)) AS ActorID ,
                ...
                CASE WHEN lastname IS NULL THEN '' ELSE ISNULL(FirstName,'') + ' ' + ISNULL(LastName,'') END AS Current_DisplayName,
                '' AS From_Actor_Title,
                ISNULL(CONVERT(VARCHAR(10), DOB, 21),'') AS DOB_ApproximateDateTime,
                CASE sex WHEN 'F' THEN 'Female' WHEN 'M' THEN 'Male' ELSE 'Unknown' END AS Gender,
        ...
        SELECT TOP ( 1 )
                REPLACE(OrganizationName,' ', '_') AS ActorID , OrganizationName AS DisplayName,
                'Organization' AS ActorRole, 'SmartcareWeb' AS SLRCGroup_Source_ActorID
        FROM    dbo.SystemConfigurations;
        
        SELECT  'SmartcareWeb' AS ActorID , 'SmartcareWeb' AS DisplayName,
                'InformationSystem' AS ActorRole, 'SmartcareWeb' AS SLRCGroup_Source_ActorID
        
        SELECT TOP(0) '' AS ActorID, '' AS ActorRole;
		
		exec ssp_CCRGetClientInfo @ClientId
		
		EXEC ssp_CCRGetMedicationListSummary @ClientId
		
		EXEC ssp_CCRGetMedicationAlertSummary @ClientId		     
		
		EXEC ssp_CCRGetLabResultsSummary @ClientId
