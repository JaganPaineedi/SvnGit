/****** Object:  StoredProcedure [dbo].[csp_RDLControlsubstanceAuditReport]    ******/
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[csp_RDLControlsubstanceAuditReport]')
			AND type IN (
				N'P'
				, N'PC'
				)
		)
	DROP PROCEDURE [dbo].[csp_RDLControlsubstanceAuditReport];
GO
...
CREATE PROCEDURE [dbo].[csp_RDLControlsubstanceAuditReport] 
	  @StartDate DATETIME
	, @EndDate DATETIME 
	, @PrescriberId int
AS
...
AS (  
 SELECT CMS.CreatedDate, CM.ClientMedicationId  
  ...
  ,CASE   
   ...
   END AS SignedByMD
  ,CASE
 WHEN CMS.ScriptEventtype = 'N'        
    THEN 'New'   
   WHEN CMS.ScriptEventtype = 'R'    
    THEN 'Refill'    
   WHEN CMS.ScriptEventtype = 'C'   
    THEN 'Change'       
   WHEN CMSA.Status = 5561        
    THEN 'Alteration'   
   END AS ScriptEventtype 
 ,CMSDS.ReadytoSign
 ,CM.ModifiedDate 
 ,'Successful'  AS StatusDescription  
 ...
 LEFT JOIN @ClientMedicationInstructionTable CMIT on LSId.ClientMedicationScriptId = CMIT.MedicationScriptId 
  ...
  LEFT JOIN ClientMedicationScriptDrugStrengths CMSDS ON (
 CMSDS.ClientMedicationScriptId = CMS.ClientMedicationScriptId
  AND ISNULL(CMSDS.RecordDeleted, 'N') = 'N'  
   )
  LEFT JOIN ClientMedicationScriptActivities CMSA ON (
 CMSDS.ClientMedicationScriptId = CMSA.ClientMedicationScriptId
...
AND (ClientMedsDistinct.CreatedDate >= (@StartDate) AND ClientMedsDistinct.CreatedDate < (@EndDate+1))
 ...
 SELECT DISTINCT CMS.CreatedDate, CM.ClientMedicationId  
  ,CM.ClientId  
  ,CS.FirstName + ' ' + CS.LastName as ClientName
  ,MDN.MedicationName AS MedicationName  
  ,CMIT.Instruction
  ...
  ,CASE   
   ...
   WHEN cst.SignatureOrder IS NULL  
    THEN 0  
   END AS SignedByMD
  ,'Ready To Sign' AS ScriptEventtype 
 ,CMSDS.ReadytoSign
 ,CMS.ModifiedDate 
 ,'Successful' AS StatusDescription  
 FROM ClientMedications CM  
 ...
 AND (  
    ISNULL(Ordered, 'N') = 'Y'  
  AND MedicationScriptID IS NOT NULL  
  AND CONVERT(INT, (ISNULL(MDDrugs.DEACODE, [dbo].[ssf_SCClientMedicationC2C5Drugs](mdmeds.MedicationId))))  > 0
AND (CMS.CreatedDate >= (@StartDate) AND CMS.CreatedDate < (@EndDate+1))
  )  
 OR (ISNULL(Ordered, 'N') = 'N' 
  AND CONVERT(INT, (ISNULL(MDDrugs.DEACODE, [dbo].[ssf_SCClientMedicationC2C5Drugs](mdmeds.MedicationId))))  > 0
AND (CMS.CreatedDate >= (@StartDate) AND CMS.CreatedDate < (@EndDate+1))
 ...
 SELECT DISTINCT CMS.CreatedDate, CM.ClientMedicationId  
  ...
  ,CASE   
   ...
   WHEN cst.SignatureOrder IS NULL  
    THEN 0  
   END AS SignedByMD
  ,'Transmission' AS ScriptEventtype 
 ,CMSDS.ReadytoSign
 ,CMS.ModifiedDate 
 ,CASE
 WHEN CMSA.Status = 5561        
...
ORDER BY CASE   
  ...
  END  
  ,t.ClientName asc
  ,t.MedicationName asc
  ,t.MedicationStartDate asc
