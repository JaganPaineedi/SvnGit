 IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLMeaningfulViewDownloadTransmitFromMUThird]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[ssp_RDLMeaningfulViewDownloadTransmitFromMUThird]
GO
...
CREATE PROCEDURE [dbo].[ssp_RDLMeaningfulViewDownloadTransmitFromMUThird] @StaffId INT          
 ,@StartDate DATETIME          
 ,@EndDate DATETIME          
 ,@MeasureType INT          
 ,@MeasureSubType INT          
 ...
 ,@Tin VARCHAR(150)            
 /********************************************************************************                  
-- Stored Procedure: dbo.ssp_RDLMeaningfulViewDownloadTransmitFromMUThird                    
--                 
...
BEGIN          
 ...
 AND NOT EXISTS (          
   ...
   AND isnull(D.RecordDeleted, 'N') = 'N' and D.DocumentCodeId in (1611 ,1644,1645,1646)      
    INNER JOIN TransitionOfCareDocuments TD  ON D.InProgressDocumentVersionId = TD.DocumentVersionId        
  ...
  WHERE  isnull(St.RecordDeleted, 'N') = 'N'          
     ...
     AND cast(D.EffectiveDate AS DATE) >= CAST(@StartDate AS DATE)                                            
       AND cast(D.EffectiveDate AS DATE) <= CAST(@EndDate AS DATE)    
        AND (TD.LocationId is not null AND ( TD.LocationId = -1 or  L.TaxIdentificationNumber=@Tin))   
...
AND (cast(P.PortalDate AS DATE) > cast(PS.DateOfService AS DATE))          
 ...
 BEGIN CATCH          
  ...
  SET @error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_RDLMeaningfulViewDownloadTransmitFromMUThird') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '
