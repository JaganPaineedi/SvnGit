IF OBJECT_ID('dbo.ssp_SCKPIReportDEExportMessagesAlerts') IS NOT NULL
  DROP PROCEDURE [dbo].[ssp_SCKPIReportDEExportMessagesAlerts]
GO
CREATE PROCEDURE [dbo].[ssp_SCKPIReportDEExportMessagesAlerts] @KPIMasterId int
AS
BEGIN TRY
  /*********************************************************************/
  ...
  ---Purpose:To get the count of DEExportMessages for a particular Period              

  ---Return:Count of DEExportMessages for a particular period             
  ...
  SELECT
    ...
    NULL AS Processed
  FROM dbo.DEExportMessages AS DE
...
BEGIN CATCH
  ...
  SET @err_msg = 'ssp_SCKPIReportDEExportMessagesAlerts: ' + ERROR_MESSAGE();
