/****** Object:  StoredProcedure [CQMSolution].[DiagnosticStudy]    Script Date: 01-02-2018 11:54:44 ******/
IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'[CQMSolution].[DiagnosticStudy]')
                    AND type IN ( N'P', N'PC' ) )
    DROP PROCEDURE [CQMSolution].[DiagnosticStudy];
GO
...
CREATE PROCEDURE [CQMSolution].[DiagnosticStudy]
...
**		Name: [CQMSolution].[DiagnosticStudy]
**		Desc: 
**
...
*******************************************************************************/
    INSERT  INTO [CQMSolution].[StagingCQMData]
            ...
            SELECT distinct @batchId ,
                    ...
                    REC_TYPE = 'CQM' ,
                    SEC_TYPE = 'DIAGNOSTICSTUDY' ,
                    D001 = NULL ,
                    D002 = '73830-2' -- LOINC CODE
                    ,
                    D003 = DC.DocumentName -- DOCUMENT NAME 
                    ,
                    D004 = '2.16.840.1.113883.6.1' ,
                    D005 = 'LOINC' ,
                    D006 = REPLACE(CONVERT(VARCHAR(10), D.EffectiveDate, 112)
                            + CONVERT(VARCHAR(8), D.EffectiveDate, 114), ':',
                            '') -- EFFECTIVEDATE
                    ,
                    D007 = REPLACE(CONVERT(VARCHAR(10), D.EffectiveDate, 112)
                            + CONVERT(VARCHAR(8), D.EffectiveDate, 114), ':',
                            '') -- EFFECTIVEDATE
                    ,
                    D008 = CASE WHEN D.DocumentId IS NULL THEN 'PRFND'
                                ELSE 'PRF'
                           END -- PERFORMED/NOT PERFORMED
                    ,
