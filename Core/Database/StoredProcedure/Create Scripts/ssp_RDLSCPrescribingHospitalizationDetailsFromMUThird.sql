 IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLSCPrescribingHospitalizationDetailsFromMUThird]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[ssp_RDLSCPrescribingHospitalizationDetailsFromMUThird]
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[ssp_RDLSCPrescribingHospitalizationDetailsFromMUThird] (            
 @StaffId INT            
 ,@StartDate DATETIME            
 ,@EndDate DATETIME            
 ,@MeasureType INT            
...
If @MeasureSubType=6267 AND (                
 ...
 BEGIN CATCH            
  ...
  ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_RDLSCPrescribingHospitalizationDetailsFromMUThird') + '*****' +       
