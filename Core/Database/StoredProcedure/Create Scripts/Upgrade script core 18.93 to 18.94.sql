----- STEP 1 ----------
IF ((select DataModelVersion FROM SystemConfigurations)  < 18.93)
BEGIN
	RAISERROR('<<< Data Model Version - Need data model version 18.93 for update.Upgrade Script Failed.>>>', 16, 1)
	RETURN 
END
------ STEP 2 ----------
...
------------------We are Adding Column in  Payers   table 

 IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
BEGIN
	IF COL_LENGTH('Payers','PayerCode') IS NOT NULL or  OBJECT_ID('Payers') IS  NULL
	BEGIN
		INSERT INTO #ColumnExists
				(value)
			VALUES (1)
		PRINT 'PayerCode Column is exists in dbo.Payers  - Skipping operation'
	END
...
BEGIN
	...
	IF COL_LENGTH('Payers','PayerCode') IS NULL
	BEGIN
		 ALTER TABLE Payers ADD PayerCode varchar(10)  NULL
	END
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN					
		IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Payers') AND type in (N'U'))     Exec sp_rename  Payers,Payers_Temp111
...
--Part1 Begin
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	exec ssp_DeleteTableChecks 'Payers_Temp111'
--Part2 Begins
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
	...
	IF EXISTS(SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'Payers_Temp111') AND is_primary_key=1)
	BEGIN
		SET @PKName=(SELECT name FROM sys.indexes WHERE object_id = OBJECT_ID(N'Payers_Temp111') AND is_primary_key=1)
...
BEGIN
 CREATE TABLE Payers(
		...
		PayerType						type_GlobalCode			NULL,			
		Active							type_Active				NULL,
		PayerCode						varchar(10)				NULL,
		RowIdentifier					type_GUID				NOT NULL,
		CONSTRAINT Payers_PK PRIMARY KEY NONCLUSTERED (PayerId)
 ) 
 IF OBJECT_ID('Payers') IS NOT NULL
	PRINT '<<< CREATED TABLE Payers >>>'
ELSE
	RAISERROR('<<< FAILED CREATING TABLE Payers >>>', 16, 1)	
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	Begin
		exec ssp_MoveForeignKeyConstraints  'Payers_Temp111','Payers'
		PRINT 'STEP 6 COMPLETED'
	end
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
		DROP TABLE Payers_Temp111
	END
...
BEGIN
	...
	IF COL_LENGTH('Programs','SNOMEDCTCode') IS NULL
	BEGIN
		 ALTER TABLE Programs ADD SNOMEDCTCode varchar(20)  NULL
	END
	--PRINT 'STEP 3 COMPLETED'	
END
...
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE  TABLE_NAME='CQMPayerConcepts')
BEGIN
/* 
 * TABLE: CQMPayerConcepts 
 */
 CREATE TABLE CQMPayerConcepts( 
		CQMPayerConceptId						int identity(1,1)	 NOT NULL,
		CreatedBy								type_CurrentUser     NOT NULL,
		CreatedDate								type_CurrentDatetime NOT NULL,
		ModifiedBy								type_CurrentUser     NOT NULL,
		ModifiedDate							type_CurrentDatetime NOT NULL,
		...
		DeletedDate								datetime             NULL,		
		PayerCode								varchar(10)			 NULL,
		PayerDescription						varchar(150)		 NULL,			
		CONSTRAINT CQMPayerConcepts_PK PRIMARY KEY CLUSTERED (CQMPayerConceptId) 
 )
 
  IF OBJECT_ID('CQMPayerConcepts') IS NOT NULL
    PRINT '<<< CREATED TABLE CQMPayerConcepts >>>'
ELSE
    RAISERROR('<<< FAILED CREATING TABLE CQMPayerConcepts >>>', 16, 1)
...
IF ((select DataModelVersion FROM SystemConfigurations)  = 18.93)
BEGIN
Update SystemConfigurations set DataModelVersion=18.94
