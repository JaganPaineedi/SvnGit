...
-- 09/25/2017		Nandita			Added logic to check if the prescription has controlled drug and ordering method is electronic for Client phone number validation
-- Nov 8 2017		Nandita			Added logic to check if atleast one phone is present for a client and not just 'Home' type	
--Nov 27 2017		Malathi Shiva   Added RecordDeleted check for "StaffLicenseDegrees" as the validation was not occurring when the NA DEA record was deleted - Thresholds - Support:#1063
--Dec 15 2017       PranayB         Added @AddressType w.r.t CEI-SupportGoLive Task#854
--Dec 18 2017       PranayB         Added Added ca.AddressType OrderBy w.r.t CEI -support Task#854
--Jan 03 2018       PranayB         Added Condition @IsControlledDrug='Y' AND @OrderingMethod='E' w.r.t Harbor 1575
-- 1/03/2018		jcarlson		Harbor Enhancement 23 - Added validation to ensure Dx Purpose is being selected
-- 01/05/2018       PranayB         Added @PhoneNumberType w.r.t KCMHSAS - Support Task	#928 - Rx.
--01/10/2018        PranayB         Commented validation for Dx.why- we have custom validation for Dx Purpose
--01/22/2018		Nandita			Removed the EndDate validation check for Nadea License. This is not required since this validation is handled in Smartcare License/Degree tab.
--07/05/2018        PranayB         Added Validation 'A valid NPI is required for all electronic scripts'  w.r.t CoreBug Task#688. 
--07/11/2018        PranayB         Added Addtional drugs to NADEA list w.r.t EPCS 75
--09/06/2018        PranayB         Added @SupervisorValidationMessage W.R.T E{CS Task# 68.
AS 
	SET nocount ON
	...
	DECLARE	@SupervisorValidationMessage NVARCHAR(MAX)
	DECLARE @AddressType INT 
    DECLARE @NPI VARCHAR(max)
	...
	END
     ...
     WHERE  ISNULL(ca.Address, '') != ''
            ...
            AND cp.ClientMedicationScriptId = @ClientMedicationScriptId
  ...
  WHERE     ISNULL(ca.PhoneNumber, '') != ''
            ...
            AND cp.ClientMedicationScriptId = @ClientMedicationScriptId
  ORDER BY  CASE WHEN ca.PhoneType = 30 THEN 1 --Home
...
--
 ...
 DECLARE @OrderingMethod CHAR(1)
	  SELECT TOP 1 @MedicationNameId=MedName.MedicationNameId,@OrderingMethod=cms.OrderingMethod FROM	ClientMedicationScriptsPreview AS cms
...
SELECT TOP 1 @DrugCategory= d.DEACode
...
DECLARE @IsControlledDrug dbo.type_YOrN
 IF @DrugCategory IN (  '2', '3', '4', '5' ) 
 BEGIN
	SET @IsControlledDrug='Y'
 END
 ELSE 
 BEGIN
	SET @IsControlledDrug='N'
 END
		...
		SELECT Top 1	@ClientValidationMessage = CASE	WHEN LEN(ISNULL(LTRIM(RTRIM(c.FirstName)),
				...
				+ CASE WHEN ISNULL(c.Sex, '') NOT IN ( 'M', 'F' )
				  ...
				  + CASE WHEN CA.Address IS NULL
					   ...
					   ELSE ''
				  END
				  + CASE WHEN CP.PhoneNumber IS NULL AND @IsControlledDrug='Y' AND @OrderingMethod='E'
					   THEN ( SELECT	dbo.Ssf_GetMesageByMessageCode(29,
															  'CLIENTSEXREQ_SCSP',
		...
		FROM	Clients AS c
				...
				LEFT JOIN ClientAddresses CA ON CA.ClientId = c.ClientId AND CA.AddressType =@AddressType AND ISNULL(ca.RecordDeleted,'N')='N'
				LEFT JOIN ClientPhones CP ON CP.ClientId = c.ClientId AND cp.PhoneType=@PhoneNumberType
		...
		SELECT @ClientMedicationId = CM.ClientMedicationId FROM  ClientMedicationScriptsPreview CMS  
                ...
                WHERE   
                         CMS.ClientMedicationScriptId = @ClientMedicationScriptId AND   ISNULL(cmi.Active,'Y')='Y'
	...
	--IF EXISTS( SELECT 1
	--			FROM  dbo.ClientMedicationScriptDrugsPreview AS b
	--			JOIN dbo.ClientMedicationInstructionsPreview AS pp ON pp.ClientMedicationInstructionId = b.ClientMedicationInstructionId
	--			JOIN ClientMedicationsPreview AS a ON a.ClientMedicationId = pp.ClientMedicationId
	--			WHERE b.ClientMedicationScriptId = @ClientMedicationScriptId
	--			AND a.DSMNumber <> 1
	--			AND ISNULL(a.RecordDeleted,'N')='N'
	--			)
	--			BEGIN
                
	--			SELECT @ClientValidationMessage = @ClientValidationMessage +  'Dx Purpose is required.';
 ...
 --Added by Nandita EPCS Task#44 validation for NA DEA for the given list of drugs
		...
		#tempMedicationName.MedicationName like  '%Suboxone%' OR #tempMedicationName.MedicationName like '%Zubsolv%' 
		...
		SELECT @NADEA=sldForNADEA.LicenseNumber FROM	ClientMedicationScriptsPreview AS cms
									...
									ISNULL(cms.RecordDeleted, 'N') <> 'Y' AND ISNULL(sldForNADEA.RecordDeleted, 'N') = 'N'
		IF(@NADEA='')
		BEGIN
    ...
    IF EXISTS ( SELECT  OrderingPrescriberId
                FROM    dbo.ClientMedicationScriptsPreview
                WHERE   OrderingMethod = 'E'
                        AND ClientMedicationScriptId = @ClientMedicationScriptId
                        AND ISNULL(RecordDeleted, 'N') = 'N' )
        BEGIN
	
            SELECT  @NPI = s.NationalProviderId
            FROM    dbo.Staff s
                    JOIN dbo.ClientMedicationScriptsPreview cmsp ON cmsp.OrderingPrescriberId = s.StaffId
            WHERE   cmsp.ClientMedicationScriptId = @ClientMedicationScriptId
                    AND ISNULL(cmsp.RecordDeleted, 'N') = 'N'
                    AND ISNULL(s.RecordDeleted, 'N') = 'N';
            IF ( ISNULL(@NPI, '') = '' )
                BEGIN
                    SET @ClientValidationMessage = ISNULL(@ClientValidationMessage,
                                                          '')
                        + ' A valid NPI is required for all electronic scripts.';
                END;
        END; 
...
SELECT  @SupervisorId = s.RxAuthorizedProvider
FROM    dbo.Staff s
WHERE   StaffId = @PresciberId;	
IF ( ISNULL(@SupervisorId,'')<> '' )
    BEGIN
        SELECT  @SupervisorValidationMessage = CASE WHEN st.PhoneNumber IS NULL
                                                         OR st.PhoneNumber = ''
                                                    THEN ( SELECT
                                                              dbo.Ssf_GetMesageByMessageCode(29,
                                                              'CLIENTSEXREQ_SCSP',
                                                              'Supervisor phone number is required on all scripts. ')
                                                         )
                                                    ELSE ''
                                               END
        FROM    dbo.Staff st
        WHERE   st.StaffId = @SupervisorId;
    END;
...
SELECT	@PrescriberValidationMessage = CASE	WHEN LEN(ISNULL(LTRIM(RTRIM(St.FirstName)),
				...
				+ CASE WHEN LEN(ISNULL(LTRIM(RTRIM(St.LastName)), '')) = 0
				  ...
				  + CASE WHEN St.Address IS NULL
					   ...
					   ELSE ''
				  END
				  + CASE WHEN (St.PhoneNumber IS NULL OR St.PhoneNumber='') and @IsControlledDrug='Y' AND @OrderingMethod='E'
					   THEN ( SELECT	dbo.Ssf_GetMesageByMessageCode(29,
															  'CLIENTSEXREQ_SCSP',
		...
		SET @ClientValidationMessage =ISNULL(@ClientValidationMessage, '')
				+  @PrescriberValidationMessage	+ISNULL(@SupervisorValidationMessage,'')	
