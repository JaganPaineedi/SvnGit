/****** Object:  StoredProcedure [dbo].[scsp_SureScriptsScriptOutput]    Script Date: 07/15/2014 23:33:12 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[scsp_SureScriptsScriptOutput]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[scsp_SureScriptsScriptOutput]
GO
...
CREATE PROCEDURE [dbo].[scsp_SureScriptsScriptOutput]
...
AS /***************************************************
    ...
    01/04/2018  PranayB     Added Days.Note- DaysSupply value is calclulated for controlled and noncontrolled.
	06/22/2018  PranayB     Added Join JOIN dbo.ClientMedicationScriptDrugsPreview csd ON csd.ClientMedicationScriptId=@ClientMedicationScriptId wr.t EPCS Task#71 
	10/25/2018 Pranay B   Added ISNULL(cmi.Active,'Y')='Y' check w.r.t Boundless 349
***************************************************/
    BEGIN TRY
	--BEGIN TRAN
                        ...
                        INSERT  INTO @DrugInfo
                                ...
                                SELECT  cmsds.StrengthId ,
                                        ...
                                        + REPLACE(REPLACE(CONVERT(VARCHAR(19), GETDATE(), 126),
                                                          ':', ''), '-', '') ,
                                        sInfo.ScriptNote ,
                                      CASE WHEN ISNULL(cmsds.SpecialInstructions,'')= '' THEN  csd.SpecialInstructions  ELSE cmsds.SpecialInstructions  END  ,
                                ...
                                FROM    ClientMedicationScriptsPreview AS cms
										...
										JOIN dbo.ClientMedicationScriptDrugsPreview csd ON csd.ClientMedicationScriptId=@ClientMedicationScriptId
                    ...
                    BEGIN
                        INSERT  INTO @DrugInfo
                                ...
                                FROM    ClientMedicationScripts AS cms
                                        ...
                                        LEFT OUTER JOIN SureScriptsRefillRequests
                                        AS ssrr ON ssrr.SureScriptsRefillRequestId = cms.SureScriptsRefillRequestId
									    JOIN dbo.ClientMedicationScriptDrugs csd ON csd.ClientMedicationScriptId=@ClientMedicationScriptId
                                        LEFT JOIN dbo.SureScriptsIncomingMessages ssim ON ( ssim.SureScriptsIncomingMessageId = ssrr.SureScriptsIncomingMessageId )
                                        LEFT JOIN dbo.SurescriptsRefillDenials ssrd ON ( ssrr.SureScriptsRefillRequestId = ssrd.SurescriptsRefillRequestId )
                                WHERE   cms.ClientMedicationScriptId = @ClientMedicationScriptId
                                        AND ISNULL(cmsds.RecordDeleted, 'N') <> 'Y'
		...
		-- get the drug description
                INSERT  INTO @ScriptOutput
                        ( ClientMedicationScriptId ,
                          ...
                          RelatesToMessageID ,
                          PotencyUnitCode,
						  TotalDaysInScript --Added by pranay
