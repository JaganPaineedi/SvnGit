----- STEP 1 ----------
IF ((select DataModelVersion FROM SystemConfigurations)  < 16.07)
BEGIN
	RAISERROR('<<< Data Model Version - Need data model version 16.07 for update.Upgrade Script Failed.>>>', 16, 1)
	RETURN 
END
------ STEP 2 -----------
...
------------------We are rearranging  Columns in  EventTypes  table 

IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
BEGIN
	IF COL_LENGTH('EventTypes','RequireProvider') IS NOT NULL or  OBJECT_ID('EventTypes') IS  NULL
	BEGIN
		INSERT INTO #ColumnExists
				(value)
			VALUES (1)
		PRINT 'RequireProvider Column is  exists in dbo.EventTypes  - Skipping operation'
	END
...
BEGIN
	IF COL_LENGTH('EventTypes','RequireProvider') IS  NULL
	BEGIN
		ALTER TABLE EventTypes ADD  RequireProvider  type_YOrN       NULL
							   CHECK (RequireProvider in ('Y','N'))
	END
END
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN					
		IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'EventTypes') AND type in (N'U'))     Exec sp_rename  EventTypes,EventTypes_Temp111
...
--Part1 Begin
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	exec ssp_DeleteTableChecks 'EventTypes_Temp111'
--Part2 Begins
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
	...
	IF EXISTS(SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'EventTypes_Temp111') AND is_primary_key=1)
	BEGIN
		SET @PKName=(SELECT name FROM sys.indexes WHERE object_id = OBJECT_ID(N'EventTypes_Temp111') AND is_primary_key=1)
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce )
BEGIN 
	IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[dbo].[DocumentCodes_EventTypes_FK]') AND parent_object_id = OBJECT_ID(N'[dbo].[EventTypes_Temp111]'))
	ALTER TABLE [dbo].[EventTypes_Temp111] DROP CONSTRAINT DocumentCodes_EventTypes_FK	
END
...
CREATE TABLE EventTypes(
	...
	AssociatedDocumentCodeId		int						NULL,
	EventName						varchar(100)			NOT NULL,
	EventType						type_GlobalCode			NOT NULL,
	DisplayNextEventGroup			type_YOrN				NULL
									CHECK (DisplayNextEventGroup in ('Y','N')),
	SummaryReportRDL				image					NULL,
	SummaryStoredProcedure			varchar(100)			NULL,
	RequireProvider					type_YOrN				NULL
									CHECK (RequireProvider in ('Y','N')),	
...
IF OBJECT_ID('EventTypes') IS NOT NULL
    PRINT '<<< CREATED TABLE EventTypes >>>'
ELSE
    RAISERROR('<<< FAILED CREATING TABLE EventTypes >>>', 16, 1)

/* 
 * TABLE: EventTypes 
 */ 

ALTER TABLE EventTypes ADD CONSTRAINT DocumentCodes_EventTypes_FK 
    FOREIGN KEY (AssociatedDocumentCodeId)
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	Begin
		exec ssp_MoveForeignKeyConstraints  'EventTypes_Temp111','EventTypes'
		PRINT 'STEP 6 COMPLETED'
	end
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
		DROP TABLE EventTypes_Temp111
	END
...
IF ((select DataModelVersion FROM SystemConfigurations)  = 16.07)
BEGIN
Update SystemConfigurations set DataModelVersion=16.08
