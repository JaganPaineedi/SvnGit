IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLClinicianTinDetails]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[ssp_RDLClinicianTinDetails]
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[ssp_RDLClinicianTinDetails] (                          
 @StaffId VARCHAR(max)                          
 ,@StartDate DATETIME                          
 ,@EndDate DATETIME                          
...
UPDATE R                              
 ...
 --    AND isnull(S.RecordDeleted, 'N') = 'N'                                            
  ...
  -- ,MeasureSubTypeId          
   , case when MeasureTypeId in (8698,8683,8697,8710,8703,9478,9479,8699) then max(cast(Numerator as int)) else sum(cast(Numerator as int)) end as 'GroupTimNumerator'                                          
   ,case when MeasureTypeId in (8698,8683,8697,8710,8703,9478,9479,8699) then max(cast(Denominator as int)) else  sum(cast(Denominator as int)) end as 'GroupTimDenominator'            
   ,Tin                                       
  FROM #ResultSet                                 
 ...
 BEGIN CATCH                                
  ...
  SET @error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), '[ssp_RDLClinicianTinDetails]') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(   
