/****** Object:  StoredProcedure [dbo].[SSP_SCGetLabsoft_SegmentDIAG]    Script Date: 04/12/2016 14:14:55 ******/
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[SSP_SCGetLabsoft_SegmentDIAG]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[SSP_SCGetLabsoft_SegmentDIAG]
GO
...
CREATE PROCEDURE [dbo].[SSP_SCGetLabsoft_SegmentDIAG] @DocumentVersionId INT
	,@EncodingChars NVARCHAR(5)
	,@DIAGSegmentRaw NVARCHAR(Max) OUTPUT
AS
/*
-- ================================================================  
-- Stored Procedure: SSP_SCGetLabsoft_SegmentDIAG  
-- Create Date : Sep 09 2015 
-- Purpose : Get DIAG Segment for Labsoft  
-- Created By : Gautam  
	declare @DIAGSegmentRaw nVarchar(max)
	exec SSP_SCGetLabsoft_SegmentDIAG 20316, '|^~\&' ,@DIAGSegmentRaw Output
	select @DIAGSegmentRaw
-- ================================================================  
-- History --  
...
BEGIN
	BEGIN TRY
		...
		IF ISNULL(@DG1Segment, '') = ''
		BEGIN
			SET @DIAGSegmentRaw = @SegmentName + @FieldChar
		END
		ELSE
		BEGIN
			SET @DIAGSegmentRaw = @SegmentName + @FieldChar + @DG1Segment
		END
	END TRY
	...
	BEGIN CATCH
		...
		SET @Error = Convert(VARCHAR, ERROR_NUMBER()) + '*****' + Convert(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + isnull(Convert(VARCHAR, ERROR_PROCEDURE()), 'SSP_SCGetLabsoft_SegmentDIAG') + '*****' + Convert(VARCHAR, ERROR_LINE()) + '*****' + Convert(VARCHAR, ERROR_SEVERITY()) + '*****' + Convert(VARCHAR, ERROR_STATE())
