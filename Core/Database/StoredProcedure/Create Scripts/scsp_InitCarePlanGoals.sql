...
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[scsp_InitCarePlanGoals]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[scsp_InitCarePlanGoals]
GO
...
CREATE PROCEDURE [dbo].[scsp_InitCarePlanGoals]
	(@CarePlanDocumentVersionID INT)
AS

/********************************************************************************                                        
-- Stored Procedure: scsp_InitCarePlanGoals                                          
--                                        
-- Copyright: Streamline Healthcate Solutions                                        
--                                        
-- Purpose: get goals and objectives for care plan document  
--                                        
-- Updates:                                                                                               
-- Date         Author      Purpose   
-- 2018.05.17   Vijay	    We are adding the custom logic, if goals end date is future date we need to show goals while initializing the care plan. 
...
BEGIN TRY
	BEGIN
		...
		SELECT CG.CarePlanGoalId
			...
			--Updated by Vijay on 17/05/2018 for CarePlan Review changes Keystone Support Go Live #118 end date logic added
			WHERE ((CG.DocumentVersionId = @CarePlanDocumentVersionID and (CG.GoalEndDate IS NULL OR CG.GoalEndDate > GETDATE()) and ISNULL(dbo.ssf_GetSystemConfigurationKeyValue('SetReviewInCarePlan'),'N') = 'Y'
		...
		SELECT TableName
			...
			,'N' as IsNewGoal
			,'N' as IsNewCarePlanGoal
		FROM #DocumentGoals

		CREATE TABLE #GoalsNeeds (
			TableName VARCHAR(30)
			,CarePlanGoalNeedId INT
			,CreatedBy VARCHAR(30)
			,CreatedDate DATETIME
			,ModifiedBy VARCHAR(30)
			,ModifiedDate DATETIME
		...
		--Updated by Vijay on 17/05/2018 for CarePlan Review changes Keystone Support Go Live #118 end date logic added
		WHERE ((ISNULL(CO.RecordDeleted, 'N') = 'N' and (CO.ObjectiveEndDate IS NULL OR CO.ObjectiveEndDate > GETDATE()) AND ISNULL(dbo.ssf_GetSystemConfigurationKeyValue('SetReviewInCarePlan'),'N')='Y')
...
BEGIN CATCH
	...
	SET @Error = Convert(VARCHAR, ERROR_NUMBER()) + '*****' + Convert(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + isnull(Convert(VARCHAR, ERROR_PROCEDURE()), 'scsp_InitCarePlanGoals') + '*****' + Convert(VARCHAR, ERROR_LINE()) + '*****' + Convert(VARCHAR, ERROR_SEVERITY()) + '*****' + Convert(VARCHAR, ERROR_STATE())
