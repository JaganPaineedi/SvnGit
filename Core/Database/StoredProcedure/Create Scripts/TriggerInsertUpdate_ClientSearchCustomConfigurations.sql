/****** Object:  Trigger [TriggerInsertUpdate_ClientSearchCustomConfigurations]    Script Date: 07/07/2016 18:37:01 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[TriggerInsertUpdate_ClientSearchCustomConfigurations]'))
DROP TRIGGER [dbo].[TriggerInsertUpdate_ClientSearchCustomConfigurations]
GO

/****** Object:  Trigger [dbo].[TriggerInsertUpdate_ClientSearchCustomConfigurations]    Script Date: 07/07/2016 18:37:01 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[TriggerInsertUpdate_ClientSearchCustomConfigurations] ON [dbo].[ClientSearchCustomConfigurations]
  FOR INSERT, UPDATE
/*********************************************************************  
-- Trigger: TriggerInsertUpdate_ServiceDropdownConfigurations
--  
...
--                           (While updating the ServiceDropdownConfigurations value it will also update the SystemConfiguratonKeys table value where SourceTableName is ServiceDropdownConfigurations)
**********************************************************************/    
AS
BEGIN
BEGIN TRY
...
WHERE object_id = object_id('ClientSearchCustomConfigurations')
...
DECLARE @TableId int
SET @TableId = (SELECT AuditLogTableId FROM dbo.AuditLogTables WHERE TableName = 'ClientSearchCustomConfigurations' AND isnull(RecordDeleted, 'N') = 'N')
...
DECLARE @Error varchar(8000)                                                                                                                                 
   ...
   + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'TriggerInsertUpdate_ClientSearchCustomConfigurations')                     
