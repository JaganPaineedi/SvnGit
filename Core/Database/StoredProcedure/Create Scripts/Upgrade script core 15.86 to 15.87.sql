----- STEP 1 ----------
IF ((select DataModelVersion FROM SystemConfigurations)  < 15.86)
BEGIN
	RAISERROR('<<< Data Model Version - Need data model version 15.86 for update.Upgrade Script Failed.>>>', 16, 1)
	RETURN 
END
------ STEP 2 ----------
...
------------------We are Adding Column in  AuthorizationCodes  table 

 IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
BEGIN
	IF COL_LENGTH('AuthorizationCodes','Category1') IS NOT NULL or  OBJECT_ID('AuthorizationCodes') IS  NULL
	BEGIN
		INSERT INTO #ColumnExists
				(value)
			VALUES (1)
		PRINT 'Category1 Column is  exists in dbo.AuthorizationCodes  - Skipping operation'
	END
	
END


IF OBJECT_ID('AuthorizationCodes')IS NOT NULL
BEGIN
	IF COL_LENGTH('AuthorizationCodes','Category1')IS NULL
	BEGIN
		ALTER TABLE AuthorizationCodes ADD Category1  type_GlobalCode  NULL
	END
	
	IF COL_LENGTH('AuthorizationCodes','Category2')IS NULL
	BEGIN
		ALTER TABLE AuthorizationCodes ADD Category2  type_GlobalCode  NULL
	END
	
	IF COL_LENGTH('AuthorizationCodes','Category3')IS NULL
	BEGIN
		ALTER TABLE AuthorizationCodes ADD Category3  type_GlobalCode  NULL
	END
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN					
		IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'AuthorizationCodes') AND type in (N'U'))     Exec sp_rename  AuthorizationCodes,AuthorizationCodes_Temp111
...
--Part1 Begin
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	exec ssp_DeleteTableChecks 'AuthorizationCodes_Temp111'
--Part2 Begins
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
	...
	IF EXISTS(SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'AuthorizationCodes_Temp111') AND is_primary_key=1)
	BEGIN
		SET @PKName=(SELECT name FROM sys.indexes WHERE object_id = OBJECT_ID(N'AuthorizationCodes_Temp111') AND is_primary_key=1)
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce )
BEGIN 
	IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[dbo].[BillingCodes_AuthorizationCodes_FK]') AND parent_object_id = OBJECT_ID(N'[dbo].[AuthorizationCodes_Temp111]'))
	ALTER TABLE [dbo].[AuthorizationCodes_Temp111] DROP CONSTRAINT BillingCodes_AuthorizationCodes_FK
END
...
BEGIN
 CREATE TABLE AuthorizationCodes(
			AuthorizationCodeId						int identity(1,1)	    NOT NULL,
			...
			DefaultBillingCodeId					int						NULL,
			AuthorizationCodeName					varchar(100)			NOT NULL,
			DisplayAs								varchar(25)				NOT NULL,
			Active									type_Active				NOT NULL
													CHECK (Active in	('Y','N')),	
			Units									decimal(18,2)			NULL,
			UnitType								type_GlobalCode			NULL,
			ProcedureCodeGroupName					varchar(100)			NULL,
			BillingCodeGroupName					varchar(100)			NULL,
			ClinicianMustSpecifyBillingCode			type_YOrN				NULL
													CHECK (ClinicianMustSpecifyBillingCode in	('Y','N')),	
			UMMustSpecifyBillingCode				type_YOrN				NULL
													CHECK (UMMustSpecifyBillingCode in	('Y','N')),	
			DefaultModifier1						varchar(10)				NULL,
			DefaultModifier2						varchar(10)				NULL,
			DefaultModifier3						varchar(10)				NULL,
			DefaultModifier4						varchar(10)				NULL,
			Internal								type_YOrN				NULL
													CHECK (Internal in	('Y','N')),	
			[External]								type_YOrN				NULL
													CHECK ([External] in	('Y','N')),	
			Category1								type_GlobalCode			NULL,
			Category2								type_GlobalCode			NULL,
			Category3								type_GlobalCode			NULL,
			RowIdentifier							type_GUID				NOT NULL,	
			CONSTRAINT AuthorizationCodes_PK PRIMARY KEY NONCLUSTERED (AuthorizationCodeId)
 ) 
 IF OBJECT_ID('AuthorizationCodes') IS NOT NULL
	PRINT '<<< CREATED TABLE AuthorizationCodes >>>'
ELSE
	RAISERROR('<<< FAILED CREATING TABLE AuthorizationCodes >>>', 16, 1)	
 ...
 */	
	...
	ALTER TABLE AuthorizationCodes ADD CONSTRAINT BillingCodes_AuthorizationCodes_FK 
		FOREIGN KEY (DefaultBillingCodeId)
...
SELECT
		...
		,Category3														
		,RowIdentifier	
	FROM [AuthorizationCodes_Temp111]
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	Begin
		exec ssp_MoveForeignKeyConstraints  'AuthorizationCodes_Temp111','AuthorizationCodes'
		PRINT 'STEP 6 COMPLETED'
	end
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
		DROP TABLE AuthorizationCodes_Temp111
	END
...
IF ((select DataModelVersion FROM SystemConfigurations)  = 15.86)
BEGIN
Update SystemConfigurations set DataModelVersion=15.87
