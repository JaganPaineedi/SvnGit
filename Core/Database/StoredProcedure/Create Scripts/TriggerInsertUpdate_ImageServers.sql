/****** Object:  Trigger [TriggerInsertUpdate_ImageServers]    Script Date: 07/07/2016 18:37:01 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[TriggerInsertUpdate_ImageServers]'))
DROP TRIGGER [dbo].[TriggerInsertUpdate_ImageServers]
GO

/****** Object:  Trigger [dbo].[TriggerInsertUpdate_ImageServers]    Script Date: 07/07/2016 18:37:01 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[TriggerInsertUpdate_ImageServers] ON [dbo].[ImageServers]
  FOR INSERT, UPDATE
/*********************************************************************  
-- Trigger: TriggerInsertUpdate_ImageServers
--  
...
WHERE object_id = object_id('ImageServers')
...
DECLARE @TableId int
SET @TableId = (SELECT AuditLogTableId FROM dbo.AuditLogTables WHERE TableName = 'ImageServers' AND isnull(RecordDeleted, 'N') = 'N')
...
DECLARE @Error varchar(8000)                                                                                                                                 
   ...
   + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'TriggerInsertUpdate_ImageServers')                     
