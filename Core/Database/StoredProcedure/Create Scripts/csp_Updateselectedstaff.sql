...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_Updateselectedstaff]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_Updateselectedstaff]
GO
...
CREATE PROC [dbo].[csp_Updateselectedstaff]     
...
/*********************************************************************/                                        
/* Stored Procedure: dbo.csp_Updateselectedstaff					 */                                        
...
BEGIN
	...
	WHERE [EPCSAssignorStaffId]=@EPCSAssignorStaffId AND [PrescriberStaffId]=@selectedStaffId
END
...
DECLARE @Error varchar(8000)                                          
   ...
   + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'csp_Updateselectedstaff ')                  
