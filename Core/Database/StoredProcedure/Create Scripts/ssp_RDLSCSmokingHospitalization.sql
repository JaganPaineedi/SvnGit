IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLSCSmokingHospitalization]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_RDLSCSmokingHospitalization]
GO
...
CREATE PROCEDURE [dbo].[ssp_RDLSCSmokingHospitalization] (    
 @StaffId INT    
 ,@StartDate DATETIME    
 ,@EndDate DATETIME    
 ,@MeasureType INT    
 ...
 /********************************************************************************        
-- Stored Procedure: dbo.ssp_RDLSCSmokingHospitalization          
--        
   ...
   ,R.Numerator = (    
    ...
    WHERE CI.STATUS <> 4981    
     ...
     AND EXISTS (    
      ...
      WHERE CDI.ClientId = CI.ClientId    
