/****** Object:  StoredProcedure [dbo].[ssp_GetDataEntererXMLString]    Script Date: 09/24/2017 13:17:53 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_GetDataEntererXMLString]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_GetDataEntererXMLString]
GO
...
CREATE PROCEDURE [dbo].[ssp_GetDataEntererXMLString] @ClientId INT = NULL        
...
BEGIN      
 ...
 DECLARE @PLACEHOLDERXML VARCHAR(MAX) = '<dataEnterer>
		<assignedEntity>
