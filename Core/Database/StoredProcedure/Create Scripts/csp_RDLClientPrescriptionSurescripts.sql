/****** Object:  StoredProcedure [dbo].[csp_RDLClientPrescriptionSurescripts]    Script Date: 10/30/2012 10:19:07 AM ******/
IF EXISTS ( SELECT  1
            FROM    INFORMATION_SCHEMA.ROUTINES
            WHERE   SPECIFIC_SCHEMA = 'dbo'
                    AND SPECIFIC_NAME = 'csp_RDLClientPrescriptionSurescripts' ) 
DROP PROCEDURE [dbo].[csp_RDLClientPrescriptionSurescripts]
GO

/****** Object:  StoredProcedure [dbo].[csp_RDLClientPrescriptionSurescripts]    Script Date: 9/13/2013 1:55:57 PM ******/
SET ANSI_NULLS ON
GO
...
SELECT @DEA=sldForNADEA.LicenseNumber FROM	ClientMedicationScriptsPreview AS cms
							...
							JOIN StaffLicenseDegrees as sldForNADEA ON sldForNADEA.StaffId=st.StaffId
							WHERE 
							sldForNADEA.LicenseTypeDegree =9403 AND sldForNADEA.PrimaryValue='Y' AND
							@ClientMedicationScriptIds = cms.ClientMedicationScriptId AND 
							ISNULL(cms.RecordDeleted, 'N') <> 'Y'
...
BEGIN
	SELECT @DEA=sldForNADEA.LicenseNumber FROM ClientMedicationScripts AS cms
