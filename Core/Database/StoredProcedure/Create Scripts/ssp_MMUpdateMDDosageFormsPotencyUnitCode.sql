/****** Object:  StoredProcedure [dbo].[ssp_MMUpdateMDDosageFormsPotencyUnitCode]    Script Date: 03/28/2016 16:39:04 ******/
IF EXISTS (SELECT *
	FROM sys.objects
	WHERE object_id = OBJECT_ID(N'[dbo].[ssp_MMUpdateMDDosageFormsPotencyUnitCode]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[ssp_MMUpdateMDDosageFormsPotencyUnitCode]
GO
...
CREATE procedure [dbo].[ssp_MMUpdateMDDosageFormsPotencyUnitCode]
--
...
as
	...
	select UpdateStatus
	,      count(*)
	from MedicationDatabase..MDDosageFormsPotencyUnitCode
	where VersionId = @VersionId
		and updatestatus in ('A', 'D')
	group by UpdateStatus

	-- Adds
	insert into MDDosageFormsPotencyUnitCode ( DosageFormId, NCPDPSCRIPTQuantityQualifierId )
	select df3.DosageFormId
	,      n3.NCPDPScriptQuantityQualifierId
	from MedicationDatabase..MDDosageFormsPotencyUnitCode   as a  
	...
	JOIN MDDosageForms                                         df3 ON df3.ExternalDosageFormId = df1.ExternalDosageFormId
			and isnull(df3.RecordDeleted, 'N') <> 'Y'
	JOIN MDDosageFormsPotencyUnitCode                          b   ON b.DosageFormId = df3.DosageFormId
