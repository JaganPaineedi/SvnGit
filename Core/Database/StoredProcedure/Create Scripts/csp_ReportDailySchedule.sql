...
/****** Object:  StoredProcedure [dbo].[csp_ReportDailySchedule]    Script Date: 09/18/2015 12:03:46 ******/
...
-- Gautam 9/18/2015 - Modified to print records Order by Time as per Richland Go Live Issues Tracking
*********************************************************************************/        
    @SelectedDate SMALLDATETIME ,        
    @ReceptionViewId INT ,        
    @Status VARCHAR(100) ,    
    @AppointmentTypes INT ,    
    @ClinicianId INT    
AS        
    ...
    INSERT  INTO #Results        
           ...
           PrimaryClinician        
            ...
            SELECT  a.clientId AS 'ClientId' ,        
                    ...
                    (CONVERT(varchar(15), CAST(a.[DateOfService] AS TIME),100) )as [DateTime],   
 ...
 ELSE                  
   ...
   Select     
    ...
    --replace(REPLACE(right('0'+ltrim(right(CONVERT(varchar,a.StartTime,100),7)),7),'AM',' AM'),'PM',' PM') As 'DateTime',   
     (CONVERT(varchar(15), CAST(a.[StartTime] AS TIME),100) )as [DateTime],   
...
*/  
    ...
    SELECT  a.ClientId ,        
            a.ClientName ,        
            --cast(replace(REPLACE(right('0'+ltrim(right(CONVERT(varchar,a.[DateTime],100),7)),7),'AM',' AM'),'PM',' PM') as time(2))  as [DateTime],  
            (CONVERT(varchar(15), CAST(a.[DateTime] AS TIME),100) )as [DateTime]  ,
             --CONVERT(varchar(15),CAST('17:30:00.0000000' AS TIME),100)   
             CAST(a.[DateTime] AS TIME) as [DateTime1]      
            ,a.MedicaidId ,        
            a.PrimaryCoverage ,        
            a.[Procedure] ,        
           a.SecondaryCoverage ,        
