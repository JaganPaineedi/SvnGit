/****** Object:  StoredProcedure [dbo].[ssp_RDLClinicianProviderDetailsFromMUThird]    Script Date: 07/03/2018 15:41:03 ******/
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLClinicianProviderDetailsFromMUThird]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[ssp_RDLClinicianProviderDetailsFromMUThird]
GO
...
CREATE PROCEDURE [dbo].[ssp_RDLClinicianProviderDetailsFromMUThird] (
	@StaffId VARCHAR(max)
	,@StartDate DATETIME
	,@EndDate DATETIME
...
BEGIN
	...
	BEGIN CATCH
		...
		SET @error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_RDLClinicianProviderDetailsFromMUThird') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())
