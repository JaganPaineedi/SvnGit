...
/****** Object:  StoredProcedure [dbo].[ssp_ProcessClientNoteProgramRequested]    Script Date: 07/26/2018 16:23:57 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_ProcessClientNoteProgramRequested]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_ProcessClientNoteProgramRequested]
GO
...
CREATE PROCEDURE [dbo].ssp_ProcessClientNoteProgramRequested (@ClientProgramId INT)
AS

/******************************************************************************                                  
**  File: ssp_ProcessClientNoteProgramRequested.sql                
**  Name: ssp_ProcessClientNoteProgramRequested           
**  Desc:                 
**                                  
...
**  Date:			Author:    Description: 
	Aug 06 2018		Ravi		Create Client Flags when Client  Requested a program   
...
INSERT INTO #TrackingProtocolList
		...
		WHERE CAST(TP.StartDate AS DATE) <= CAST(CP.RequestedDate AS DATE)
			AND (TP.EndDate IS NULL OR (CAST(TP.EndDate AS DATE) >= CAST(CP.RequestedDate AS DATE)))
			AND CP.ClientProgramId=@ClientProgramId
			AND CP.[Status]=1--Requested
			AND (TPF.FlagAssignment = 'M'
				...
				WHERE CPs.ClientProgramId = @ClientProgramId
					AND (CTTM.StartDate IS NULL OR (CAST(CTTM.StartDate AS DATE) <= CAST(CPs.RequestedDate AS DATE))) 
					AND (CTTM.EndDate IS NULL OR (CAST(CTTM.EndDate AS DATE) >= CAST(CPs.RequestedDate AS DATE)))
			...
			AND GC3.Code='On Program Requested'
...
END TRY
	...
	BEGIN CATCH
		...
		SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_ProcessClientNoteProgramRequested') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())
