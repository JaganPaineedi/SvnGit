/****** Object:  StoredProcedure [dbo].[ssp_SCCreateOrderTemplateFromMessages]    Script Date: 12/18/2018 7:17:46 AM ******/

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_SCCreateOrderTemplateFromMessages]') AND type in (N'P', N'PC'))
BEGIN
DROP PROCEDURE [dbo].[ssp_SCCreateOrderTemplateFromMessages] 
END
GO
...
CREATE PROCEDURE [dbo].[ssp_SCCreateOrderTemplateFromMessages] @MessageXML XML
...
BEGIN
	...
	BEGIN CATCH
		...
		SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'ssp_SCCreateOrderTemplateFromMessages') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())
