----- STEP 1 ----------
IF ((select DataModelVersion FROM SystemConfigurations)  < 16.59)
BEGIN
	RAISERROR('<<< Data Model Version - Need data model version 16.59 for update.Upgrade Script Failed.>>>', 16, 1)
	RETURN 
END
------ STEP 2 ----------
--Part1 Begin
...
------------------We are Adding Column in  ClientAddressHistory  table 

 IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
BEGIN
	IF COL_LENGTH('ClientAddressHistory','StartDate') IS NOT NULL or  OBJECT_ID('ClientAddressHistory') IS  NULL
	BEGIN
		INSERT INTO #ColumnExists
				(value)
			VALUES (1)
		PRINT 'StartDate Column is  exists in dbo.ClientAddressHistory  - Skipping operation'
	END
...
------ STEP 3 ------------
IF OBJECT_ID('ClientAddressHistory')  IS NOT NULL
BEGIN
	IF COL_LENGTH('ClientAddressHistory','StartDate') IS NULL
	BEGIN
		ALTER TABLE ClientAddressHistory ADD StartDate  datetime NULL
	END
	
	IF COL_LENGTH('ClientAddressHistory','EndDate') IS NULL
	BEGIN
		ALTER TABLE ClientAddressHistory ADD EndDate  datetime NULL
	END
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN					
		IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'ClientAddressHistory') AND type in (N'U'))     Exec sp_rename  ClientAddressHistory,ClientAddressHistory_Temp111
...
--Part1 Begin
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	exec ssp_DeleteTableChecks 'ClientAddressHistory_Temp111'
--Part2 Begins
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
	...
	IF EXISTS(SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'ClientAddressHistory_Temp111') AND is_primary_key=1)
	BEGIN
		SET @PKName=(SELECT name FROM sys.indexes WHERE object_id = OBJECT_ID(N'ClientAddressHistory_Temp111') AND is_primary_key=1)
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce )
BEGIN 
	IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[dbo].[Clients_ClientAddressHistory_FK]') AND parent_object_id = OBJECT_ID(N'[dbo].[ClientAddressHistory_Temp111]'))
	ALTER TABLE [dbo].[ClientAddressHistory_Temp111] DROP CONSTRAINT Clients_ClientAddressHistory_FK	
END

------ STEP 4 ----------

/* 
 * TABLE: ClientAddressHistory 
 */
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
BEGIN
 CREATE TABLE ClientAddressHistory(
		...
		AddressType							type_GlobalCode					NOT NULL,	
		[Address]							type_Address_Encrypted			NULL,
		City								type_City_Encrypted				NULL,
		[State]								type_State						NULL,
		Zip									type_ZipCode_Encrypted			NULL,
		Display								type_AddressDisplay_Encrypted	NULL,
		Billing								type_YOrN						NULL
											CHECK (Billing in ('Y','N')),	
		ExternalReferenceId					type_ExternalReferenceId		NULL,	
		StartDate							datetime						NULL,
		EndDate								datetime						NULL,
		RowIdentifier						type_GUID						NOT NULL,
		CONSTRAINT ClientAddressHistory_PK PRIMARY KEY CLUSTERED (ClientAddressId)
 ) 
 IF OBJECT_ID('ClientAddressHistory') IS NOT NULL
	PRINT '<<< CREATED TABLE ClientAddressHistory >>>'
ELSE
	RAISERROR('<<< FAILED CREATING TABLE ClientAddressHistory >>>', 16, 1)	
	
/* 
 * TABLE: ClientAddressHistory 
 */	
 
ALTER TABLE ClientAddressHistory ADD CONSTRAINT Clients_ClientAddressHistory_FK 
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
BEGIN
SET IDENTITY_INSERT ClientAddressHistory ON	
...
SELECT
	...
	,EndDate
	,RowIdentifier
	FROM [ClientAddressHistory_Temp111]
	SET IDENTITY_INSERT ClientAddressHistory OFF	
END
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	Begin
		exec ssp_MoveForeignKeyConstraints  'ClientAddressHistory_Temp111','ClientAddressHistory'
		PRINT 'STEP 6 COMPLETED'
	end
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
		DROP TABLE ClientAddressHistory_Temp111
	END
...
IF ((select DataModelVersion FROM SystemConfigurations)  = 16.59)
BEGIN
Update SystemConfigurations set DataModelVersion=16.60
