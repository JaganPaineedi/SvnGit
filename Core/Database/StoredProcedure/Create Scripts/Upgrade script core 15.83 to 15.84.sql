----- STEP 1 ----------
IF ((select DataModelVersion FROM SystemConfigurations)  < 15.83)
BEGIN
	RAISERROR('<<< Data Model Version - Need data model version 15.83 for update.Upgrade Script Failed.>>>', 16, 1)
	RETURN 
END
------ STEP 2 ----------
--Part1 Begin
...
------------------We are Adding Column in  ClientPhones  table 

 IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
BEGIN
	IF COL_LENGTH('ClientPhones','DoNotLeaveMessage') IS NOT NULL or  OBJECT_ID('ClientPhones') IS  NULL
	BEGIN
		INSERT INTO #ColumnExists
				(value)
			VALUES (1)
		PRINT 'DoNotLeaveMessage Column is  exists in dbo.ClientPhones  - Skipping operation'
	END
	
END


IF OBJECT_ID('ClientPhones') IS NOT NULL
BEGIN
	IF COL_LENGTH('ClientPhones','DoNotLeaveMessage') IS  NULL
	BEGIN
		ALTER TABLE ClientPhones ADD  DoNotLeaveMessage  type_YOrN	NULL
								 CHECK (DoNotLeaveMessage in('Y','N'))
	END
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN					
		IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'ClientPhones') AND type in (N'U'))     Exec sp_rename  ClientPhones,ClientPhones_Temp111
...
--Part1 Begin
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	exec ssp_DeleteTableChecks 'ClientPhones_Temp111'
--Part2 Begins
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
	...
	IF EXISTS(SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID(N'ClientPhones_Temp111') AND is_primary_key=1)
	BEGIN
		SET @PKName=(SELECT name FROM sys.indexes WHERE object_id = OBJECT_ID(N'ClientPhones_Temp111') AND is_primary_key=1)
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce )
BEGIN 
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[ClientPhones_Temp111]') AND name = N'XIE1_ClientPhones')
	BEGIN
	DROP INDEX [XIE1_ClientPhones] ON [dbo].[ClientPhones_Temp111] 
	END	
END

IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce )
BEGIN 
	IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[dbo].[ClaimFormats_ClientPhones_FK]') AND parent_object_id = OBJECT_ID(N'[dbo].[ClientPhones_Temp111]'))
	ALTER TABLE [dbo].[ClientPhones_Temp111] DROP CONSTRAINT ClaimFormats_ClientPhones_FK
END

IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce )
BEGIN 
	IF  EXISTS (SELECT * FROM sys.foreign_keys WHERE object_id = OBJECT_ID(N'[dbo].[Clients_ClientPhones_FK]') AND parent_object_id = OBJECT_ID(N'[dbo].[ClientPhones_Temp111]'))
	ALTER TABLE [dbo].[ClientPhones_Temp111] DROP CONSTRAINT Clients_ClientPhones_FK
END
...
BEGIN
 CREATE TABLE ClientPhones(
				...
				PhoneType								type_GlobalCode			NULL,
				PhoneNumber								type_PhoneNumberEncrypted	NOT NULL,
				PhoneNumberText							varchar(25)				NULL,
				IsPrimary								type_YOrN				NULL
														CHECK (IsPrimary in	('Y','N')),	
				DoNotContact							type_YOrN				NULL
														CHECK (DoNotContact in	('Y','N')),	
				ExternalReferenceId						type_ExternalReferenceId	NULL,
				DoNotLeaveMessage						type_YOrN				NULL
														CHECK (DoNotLeaveMessage in	('Y','N')),					
				RowIdentifier							type_GUID				NOT NULL,			
				CONSTRAINT ClientPhones_PK PRIMARY KEY CLUSTERED (ClientPhoneId)
 ) 
 IF OBJECT_ID('ClientPhones') IS NOT NULL
	PRINT '<<< CREATED TABLE ClientPhones >>>'
ELSE
	RAISERROR('<<< FAILED CREATING TABLE ClientPhones >>>', 16, 1)	

 
 IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[ClientPhones]') AND name = N'XIE1_ClientPhones')
	BEGIN
		CREATE NONCLUSTERED INDEX [XIE1_ClientPhones] ON [dbo].[ClientPhones] 
		(
		[ClientId] asc
		)
		WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
		IF EXISTS (SELECT * FROM sys.indexes WHERE object_id=OBJECT_ID('ClientPhones') AND name='XIE1_ClientPhones')
		PRINT '<<< CREATED INDEX ClientPhones.XIE1_ClientPhones >>>'
		ELSE
		RAISERROR('<<< FAILED CREATING INDEX ClientPhones.XIE1_ClientPhones >>>', 16, 1)		
	END
 ...
 */	
	...
	ALTER TABLE ClientPhones ADD CONSTRAINT Clients_ClientPhones_FK 
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
BEGIN
SET IDENTITY_INSERT ClientPhones ON	
...
SELECT
	...
	,DoNotLeaveMessage
	,RowIdentifier		
	FROM [ClientPhones_Temp111]
	SET IDENTITY_INSERT ClientPhones OFF	
END
...
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	Begin
		exec ssp_MoveForeignKeyConstraints  'ClientPhones_Temp111','ClientPhones'
		PRINT 'STEP 6 COMPLETED'
	end
IF NOT EXISTS ( SELECT 1 FROM #ColumnExists ce ) 
	BEGIN
		DROP TABLE ClientPhones_Temp111
	END
...
IF ((select DataModelVersion FROM SystemConfigurations)  = 15.83)
BEGIN
Update SystemConfigurations set DataModelVersion=15.84
