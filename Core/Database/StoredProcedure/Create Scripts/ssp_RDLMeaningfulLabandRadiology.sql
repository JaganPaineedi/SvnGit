 IF EXISTS (
		SELECT 1
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[ssp_RDLMeaningfulLabandRadiology]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[ssp_RDLMeaningfulLabandRadiology]
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[ssp_RDLMeaningfulLabandRadiology]         
 @StaffId INT        
 ,@StartDate DATETIME        
 ,@EndDate DATETIME        
 ,@MeasureType INT        
...
END     
 ...
 BEGIN CATCH        
    ...
    'ssp_RDLMeaningfulLabandRadiology')        
