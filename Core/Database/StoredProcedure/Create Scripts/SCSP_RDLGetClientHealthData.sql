IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SCSP_RDLGetClientHealthData]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SCSP_RDLGetClientHealthData]
GO
SET ANSI_NULLS ON
GO
...
GO     
CREATE Procedure [dbo].[SCSP_RDLGetClientHealthData]           
(          
 @ClientId INT,          
...
/********************************************************************************                                                                    
-- Stored Procedure: dbo.SCSP_RDLGetClientHealthData.sql                                                                      
--  SCSP_RDLGetClientHealthData 4,110                                                                 
...
; with cte as    
(    
   SELECT distinct t.HealthDataAttributeId, t.value ,t.HealthRecordDate ,t.ValueWithUnits,t.HealthDataSubTemplateName,t.Description,CASE WHEN ((H.MinimumValue is null) OR (H.MaximumValue is null)) THEN 'Black'  ELSE H.Color end as Color ,H.MaximumValue,h 
 
.MinimumValue,t.HealthRecordDateTemp,t.EntryDisplayOrder,t.OrderInFlowSheet FROM #TemplateResult T left join #HealthDataGraphCriteriaRanges H ON T.HealthDataAttributeId=H.HealthDataAttributeId where ISNUMERIC(value)=1    
...
MinimumValue,t.HealthRecordDateTemp,t.EntryDisplayOrder,t.OrderInFlowSheet FROM #TemplateResult T left join #HealthDataGraphCriteriaRanges H ON T.HealthDataAttributeId=H.HealthDataAttributeId where ISNUMERIC(value)=0    
...
HealthDataAttributeId ,      
value ,      
CONVERT(VARCHAR(19),HealthRecordDate,101)+' '+CONVERT(varchar(15),  CAST(HealthRecordDate AS TIME), 100) as HealthRecordDate     ,      
ValueWithUnits ,    
HealthDataSubTemplateName ,    
[Description] ,    
Color ,    
MinimumValue ,    
MaximumValue ,    
HealthRecordDateTemp ,    
EntryDisplayOrder ,    
OrderInFlowSheet   

 FROM  #FinalResult order by  cast(HealthRecordDate as datetime) desc,EntryDisplayOrder ,OrderInFlowSheet    
...
END TRY              
 ...
 BEGIN CATCH                  
  ...
  SET @Error= CONVERT(VARCHAR,ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000),ERROR_MESSAGE())                                                                                                              
   + '*****' + ISNULL(CONVERT(VARCHAR,ERROR_PROCEDURE()),'SCSP_RDLGetClientHealthData.sql')                                                                                                               
