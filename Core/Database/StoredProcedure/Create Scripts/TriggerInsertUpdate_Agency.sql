/****** Object:  Trigger [TriggerInsertUpdate_Agency]    Script Date: 07/07/2016 18:37:01 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[TriggerInsertUpdate_Agency]'))
DROP TRIGGER [dbo].[TriggerInsertUpdate_Agency]
GO

/****** Object:  Trigger [dbo].[TriggerInsertUpdate_Agency]    Script Date: 07/07/2016 18:37:01 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[TriggerInsertUpdate_Agency] ON [dbo].[Agency]
  FOR INSERT, UPDATE
/*********************************************************************  
-- Trigger: TriggerInsertUpdate_Agency
--  
...
WHERE object_id = object_id('Agency')
...
DECLARE @TableId int
SET @TableId = (SELECT AuditLogTableId FROM dbo.AuditLogTables WHERE TableName = 'Agency' AND isnull(RecordDeleted, 'N') = 'N')
...
DECLARE @Error varchar(8000)                                                                                                                                 
   ...
   + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'TriggerInsertUpdate_Agency')                     
