...
IF EXISTS (SELECT * 
           FROM   sys.objects 
           WHERE  object_id = 
                  Object_id(N'[dbo].[ssp_SCGetClientMedicationDetails]' 
                  ) 
...
/**********************************************************************/ 
  BEGIN 
      ...
      FROM   ClientMedications CM 
             LEFT JOIN ClientMedicationInstructions CMI 
                    ON CMI.ClientMedicationId = CM.ClientMedicationId 
                       AND ISNULL(CMI.RecordDeleted, 'N') = 'N' 
             LEFT JOIN ClientMedicationScriptDrugs CMSD 
                    ON CMI.ClientMedicationInstructionId = 
                       CMSD.ClientMedicationInstructionId 
                       AND ISNULL(CMSD.RecordDeleted, 'N') = 'N' 
             LEFT JOIN ClientMedicationScripts CMS 
                    ON CMS.ClientMedicationScriptId = 
      ...
      SELECT DISTINCT cm.ClientMedicationId, 
                      ...
                      CASE 
                        WHEN ISNULL(CM.Discontinued, 'N') = 'Y' 
                             AND ISNULL(CM.Ordered, 'N') = 'Y' THEN 
                        ISNULL(cm.PrescriberName, cm.ExternalPrescriberName) 
                        ELSE cms.OrderingPrescriberName 
                      END                                          AS 
                      PrescribedByName 
                      , 
                      t.MedicationStartDate                        AS 
                      MedicationStartDate, 
                      t.MedicationEndDate                          AS 
                      MedicationEndDate, 
                      ISNULL(cm.DSMCode, '0') + '_' 
                      + ISNULL(Cast(cm.DSMNumber AS VARCHAR), '0') AS DxId, 
                      mdn.MedicationName, 
                      CASE 
                      --  WHEN ISNULL(CM.Discontinued, 'N') = 'Y' 
                      ...
                      --THEN 'Discontinued' 
                        WHEN ISNULL(CMS.Voided, 'N') = 'Y' THEN 'Voided' 
                        ELSE 
                          CASE CMS.ScriptEventType 
                            WHEN 'N' THEN 'New' 
                            WHEN 'C' THEN 'Changed' 
                      ...
                      END                                          AS 
                      OrderStatusDate, 
                      cms.ClientMedicationScriptId                 AS 
                      MedicationScriptId, 
                      ...
                      CASE 
                        WHEN ISNULL(CM.Discontinued, 'N') = 'Y' 
                             AND ISNULL(CM.Ordered, 'N') = 'Y' THEN 'Y' 
      ...
      FROM   ClientMedications cm 
             JOIN MDMedicationNames mdn 
               ON ( mdn.MedicationNameId = cm.MedicationNameId 
             ...
             JOIN ClientMedicationScripts cms 
               ON cms.ClientMedicationScriptId = cmsd.ClientMedicationScriptId 
                  AND ISNULL(cms.RecordDeleted, 'N') = 'N' 
             JOIN #temp t 
               ON ( t.Clientmedicationid = cm.Clientmedicationid 
      ...
      SELECT DISTINCT cm.ClientMedicationId, 
                      ...
                      END                                          AS Comments, 
                      cm.SmartCareOrderEntry ,
					  ROW_NUMBER()OVER(PARTITION BY cm.MedicationNameId ORDER BY cm.MedicationNameId ASC ,cms.ClientMedicationScriptId desc) AS RowCountNo
      FROM   ClientMedications cm 
             JOIN MDMedicationNames mdn 
               ON ( mdn.MedicationNameId = cm.MedicationNameId 
                    AND ISNULL(mdn.RecordDeleted, 'N') = 'N' ) 
      ...
      WHERE  cm.ClientId = @ClientId 
             ...
             AND ISNULL(cm.Ordered, 'N') = 'Y' 
             AND ISNULL(cms.WaitingPrescriberApproval, 'N') = 'N' 
             AND ISNULL(Discontinued,'N') = 'Y')Main where RowCountNo=1
    ...
    UNION
      ...
      SELECT DISTINCT cm.ClientMedicationId, 
                      ...
                      cm.DeletedBy, 
                      mdn.MedicationName, 
                      ISNULL(cm.PrescriberName, cm.ExternalPrescriberName) AS 
                      PrescribedByName, 
                      --cm.MedicationStartDate , 
                      t.MedicationStartDate                                AS 
                      MedicationStartDate, 
                      ...
                      ISNULL(cm.DSMCode, '0') + '_' 
                      + ISNULL(Cast(cm.DSMNumber AS VARCHAR), '0')         AS 
                      DxId 
                      , 
                      mdn.MedicationName 
                      AS 
                      ...
                      cm.Comments,-- Added by Loveena in ref to Task#32       
                      cm.SmartCareOrderEntry 
      FROM   ClientMedications CM 
             JOIN MDMedicationNames mdn 
               ON ( mdn.MedicationNameId = cm.MedicationNameId 
                    AND ISNULL(mdn.RecordDeleted, 'N') = 'N' ) 
             LEFT OUTER JOIN ClientMedicationInstructions cmi 
                          ON cmi.ClientMedicationId = cm.ClientMedicationId 
                             AND ISNULL(cmi.RecordDeleted, 'N') = 'N' 
             JOIN #temp t 
               ON ( t.Clientmedicationid = cm.Clientmedicationid 
      ...
      FROM   ClientMedicationInstructions cmi 
             JOIN #results AS r 
               ON ( r.clientmedicationId = cmi.clientMedicationid ) 
             LEFT JOIN GlobalCodes GC 
                    ON ( GC.GlobalCodeID = cmi.Unit ) 
             LEFT JOIN GlobalCodes GC1 
                    ON ( GC1.GlobalCodeId = cmi.Schedule ) 
             JOIN MDMedications MD 
               ON ( MD.MedicationID = cmi.StrengthId 
