...
function makeUri() {
    ...
    if (isIE()) {
        if (prot == 'https:') {
            return (prot + "//127.0.0.1:47290/SigWeb/");
        }
        else {
            return (prot + "//127.0.0.1:47289/SigWeb/");
        }
    }
    ...
    if (isChrome()) {
        ...
        //FIREFOX
        if (prot == 'https:') {
            return (prot + "//localhost:47290/SigWeb/");
            //return( prot + "//127.0.0.1:47290/SigWeb/" );
        }
        else {
            return (prot + "//localhost:47289/SigWeb/");
            //return( prot + "//127.0.0.1:47290/SigWeb/" );
        }
    }
}
...
//Function to check whether new Sigweb installed on a local machine or not.
function SigWebInstalled() {
    try {
        var xhr = SigWebcreateXHR();

        if (xhr) {
            xhr.open("POST", baseUri, false);
            xhr.send(null);
            if (xhr.readyState == 4 && xhr.status == 200) {
                return xhr.responseText;
            }
...
function onSignChrome() {
    try {
        var ctx = document.getElementById('canvasSignPad').getContext('2d');
