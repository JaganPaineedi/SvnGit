...
function onSuccessfullDeletion(result, context, methodname) {
    try {
        ...
        parent.window.redirectToOrderDetailsPage('PatientConsentHistory');
...
function ViewFillMedicationControl(Panel, SortBy) {
    ...
    try {
        ...
        wRequest.set_url("AjaxScript.aspx?FunctionId=GetClientConsentHistory&par0=" + SortBy);
...
function onConsentHistoryHeaderClick(obj) {
    try {
        ...
        GetConsentMedicationViewList(PanelMedicationViewList, ColumnName);
...
function GetConsentMedicationViewList(Panel, SortBy) {

    try {

        requestUrl = "AjaxScript.aspx?FunctionId=GetClientConsentHistory&par0=" + SortBy;
        //alert(requestUrl);

        CreateXmlHttp();
        if (XmlHttp) {
            XmlHttp.onreadystatechange = function() { handleGetConsentMedicationList(Panel) }
...
function handleGetConsentMedicationList(PanelConsentMedicationViewList) {
    try {
        if (XmlHttp.readyState == 4) {
            if (XmlHttp.status == 200) {
                PanelConsentMedicationViewList.innerHTML = "";
                ...
                if (data.length > 545) {
                    ...
                    var Rscript = document.createElement('script');
                    Rscript.text = Script;
                    //--Comented by Pradeep for time being--PanelConsentMedicationViewList.appendChild(Rscript);
                    //                var startIndex=data.indexOf('<ConsentMedList>')+16;
                    //                var endIndex = data.indexOf('</ConsentMedList>');
                    var startIndex = data.indexOf('<consentmedlist>') + 16;
                    var endIndex = data.indexOf('</consentmedlist>');
                    //data = data.substring(data.indexOf('<ConsentMedList>')+10,data.indexOf('</ConsentMedList>'));
                    data = data.substring(startIndex, endIndex);
                    PanelConsentMedicationViewList.innerHTML = data + PanelConsentMedicationViewList.innerHTML;
...
function ButtonPrintListViewConsentHistory(stringReportName) {
    try {
        //fnShow(); //By Vikas Vyas On Dated April 04th 2008
        var StartDate = $("[id$=TextBoxConsentStartDate]").val()
        var EndDate = $("[id$=TextBoxConsentEndDate]").val();
        var MedicationNameId = $("[id$=DropDownConsentMedication]").val();

        if (EndDate == "" || EndDate == undefined)
            EndDate = "01/01/9999";

        if (MedicationNameId == "0" || MedicationNameId == undefined)
            MedicationNameId = "null";

        //Streamline.SmartClient.WebServices.CommonService.GetSystemReportsMedHistory(stringReportName, StartDate, EndDate, MedicationNameId, onSuccessfullGetSystemReportsMedHistory, onFailure);
        PopupCenter("LoadReport.aspx?reportName=" + encodeURIComponent(stringReportName) + "&StartDate=" + StartDate + "&EndDate=" + EndDate + "&MedicationNameId=" + MedicationNameId, "SmartCare", 760, 500); 
    }
    catch (e) {
