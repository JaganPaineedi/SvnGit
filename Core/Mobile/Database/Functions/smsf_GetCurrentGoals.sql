...
GO
CREATE FUNCTION [dbo].[smsf_GetCurrentGoals]
(@ClientId INT
)
RETURNS VARCHAR(MAX)
AS
     -- =============================================      -- Author:  Shankha B      -- Create date: Apr-20-2017-- Description: Get Goals and Objectives for multiple Customers based on INITIALIZEGOALSOBJECTIVESFORTYPE key
/*      
 Author			Modified Date			Reason      
 Pradeep			May-03-2017		     Added CDCPG.DocumentVersionId to the Thresholds flavor to take care of Order By Clause.
 Pradeep			Dec-05-2017		     Added Custom section for Harbor Goals and Objectives
*/

     -- =============================================   
     BEGIN
         DECLARE @JsonResult VARCHAR(MAX), @ConfiguredType VARCHAR(100);
         DECLARE @RecentDocumentVersionId AS INT, @RecentDocumentEffectiveDate DATETIME;
         DECLARE @PreviousDocumentVersionId AS INT;
         DECLARE @EpisodeRegistrationDate DATE;
         DECLARE @HasCarePlan CHAR(1);
         DECLARE @CarePlanEffectiveDate DATETIME, @GoalsEffectiveDate DATETIME;
             ...
             BEGIN
                 ...
                 (
                     ...
                     WHERE T.DocumentVersionId = @RecentDocumentVersionId
                           ...
                           AND T.GoalActive = 'Y' FOR XML PATH, ROOT
                 )
                 );
             END;
         ELSE
         IF @ConfiguredType = 'HARBOR'
             BEGIN
                 SELECT TOP 1 @RecentDocumentVersionId = CurrentDocumentVersionId
                 FROM documents
                 ...
                 AND ClientId = @ClientId
                 AND status = 22
                 AND DATEDIFF(DAY, documents.EffectiveDate, GETDATE()) >= 0
                 AND isnull(RecordDeleted, 'N') = 'N' ORDER BY effectivedate DESC,
                                                               DocumentId DESC;
                 SELECT @JsonResult = dbo.smsf_FlattenedJSON
                 (
                 (
                     SELECT tpg.TPGoalId AS GoalId,
                            tpg.GoalNumber,
                            tpg.GoalText,
                            tpg.Active AS CustomGoalActive,
                            NULL AS ProcedureCodeId,
                            NULL AS ProgramId,
                            d.EffectiveDate AS EffectiveDate
                     FROM dbo.CustomTPGoals AS tpg
                          JOIN Documents d ON d.CurrentDocumentVersionId = tpg.DocumentVersionId
                     WHERE DocumentVersionId = @RecentDocumentVersionId
                           AND Isnull(D.RecordDeleted, 'N') = 'N'
                           AND Isnull(tpg.RecordDeleted, 'N') = 'N'
                           AND ISNULL(tpg.Active, 'Y') = 'Y' FOR XML PATH, ROOT
                 )
                 );
             END;
         ELSE
         IF @ConfiguredType = 'PHILHAVEN'
             BEGIN
                 ...
                 (
                     ...
                     WHERE d.ClientId = @ClientID
                           ...
                           AND isNull(d.RecordDeleted, 'N') <> 'Y' ORDER BY d.EffectiveDate DESC,
                                                                            d.ModifiedDate DESC
                 );
                 SELECT @JsonResult = dbo.smsf_FlattenedJSON
                 (
                 (
                     ...
                     WHERE ISNull(CG.RecordDeleted, 'N') = 'N'
                           ...
                           AND CG.DocumentVersionId = @LatestCarePlanDocumentVersionID FOR XML PATH, ROOT
                 )
                 );
             END;
         ELSE
         IF @ConfiguredType = 'THRESHOLDS'
             BEGIN
                 ...
                 (
                     ...
                     FROM Documents d
                          LEFT JOIN CustomDocumentCarePlans cp ON cp.DocumentVersionId = d.CurrentDocumentVersionId
                                                                  AND ISNULL(cp.CarePlanType, 'IN') IN('IN', 'AN')
                 ...
                 IF @HasCarePlan = 'Y'
                     BEGIN
                         ...
                         ISNULL(d.RecordDeleted, 'N') = 'N' ORDER BY d.EffectiveDate DESC,
                                                                     d.DocumentId DESC;
                         IF CAST(@RecentDocumentEffectiveDate AS DATE) = CAST(GETDATE() AS DATE)
                             BEGIN
                                 ...
                                 FROM Documents d
                                 WHERE d.DocumentCodeId IN(1501, 10514)
                                 AND -- Care Plan or Converted Care Plan      
                                 d.ClientId = @ClientId
                                 AND d.STATUS = 22
                                 AND -- Signed      
                                 ...
                                 CAST(DATEADD(month, 6, d.EffectiveDate) AS DATE) >= CAST(GETDATE() AS DATE)
                                 AND ISNULL(d.RecordDeleted, 'N') = 'N'
                                 AND d.CurrentDocumentVersionId <> @RecentDocumentVersionId ORDER BY d.EffectiveDate DESC,
                                                                                                     d.DocumentId DESC;
                             END;
                     END;
                 ...
                 (
                     ...
                     WHERE CDCPG.DocumentVersionId IN(@PreviousDocumentVersionId, @RecentDocumentVersionId)
                     ...
                     (
                         ...
                         WHERE CDCPG.DocumentVersionId = @PreviousDocumentVersionId
                               ...
                               AND CDCPG.DomainGoalId = CDCPG2.DomainGoalId
                                ...
                                CDCPG.GoalNumber FOR XML PATH, ROOT
                 )
                 );
             END;
         ELSE
             BEGIN
                 ...
                 (
                     ...
                     WHERE d.ClientId = @ClientID
                           ...
                           AND isNull(d.RecordDeleted, 'N') = 'N'
                           AND ISNull(DCP.RecordDeleted, 'N') = 'N' ORDER BY d.EffectiveDate DESC,
                                                                             d.ModifiedDate DESC
                 );
                     ...
                     BEGIN
                         ...
                         (
                             ...
                             WHERE ISNull(CG.RecordDeleted, 'N') = 'N'
                                   ...
                                   AND CAST(isnull(CG.GoalEndDate, GETDATE()) AS DATE) >= CAST(GETDATE() AS DATE) ORDER BY CG.GoalNumber FOR XML PATH, ROOT
                         )
                         );
                     END;
             END;
