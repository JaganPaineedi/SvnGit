...
function ShowHide() {
    var status = $("#DropDownList_CustomDocumentTransfers_TransferStatus option:selected").text().toLowerCase();
    ...
    if (status == 'sent') {
        ...
        $("#DropDownList_CustomDocumentTransferServices_AuthorizationCodeId").attr('disabled', false);
        ...
        $("#DropDownList_CustomDocumentTransfers_ReceivingProgram").attr('disabled', false);
    ...
    if (status == 'not sent') {
        ...
        // $("#DropDownList_CustomDocumentTransfers_TransferStatus").attr('disabled', true);
        $("#DropDownList_CustomDocumentTransfers_TransferringStaff").attr('disabled', true);
        $("#TextArea_CustomDocumentTransfers_AssessedNeedForTransfer").attr('disabled', true);
        $("#DropDownList_CustomDocumentTransferServices_AuthorizationCodeId").attr('disabled', true);
...
function BindProgramDropDown() {
    ...
    var StaffId = $("#DropDownList_CustomDocumentTransfers_ReceivingStaff").val();
    var strDatasource;
    $.ajax({
        type: "POST",
    ...
    addDropDownValue(strDatasource); 
      var txtVal = $("#TextBox_CustomDocumentTransfers_ReceivingProgram").val();

      if (txtVal != "" && txtVal!="-1") {
        $("#DropDownList_CustomDocumentTransfers_ReceivingProgram").val(txtVal);
    }
...
//    }
    ...
    function SetValue() {
        var stafprogamId = $("#DropDownList_CustomDocumentTransfers_ReceivingProgram").val();
        if (stafprogamId != null && stafprogamId!="-1") {
            $("#TextBox_CustomDocumentTransfers_ReceivingProgram").val(stafprogamId);
        }
        else {
    ...
    function addDropDownValue(result) {
        $("#DropDownList_CustomDocumentTransfers_ReceivingProgram").children().remove();
        // Assign text and value to Option object

        if (result != null || result != '') {
            if (result.xml != null || result.xml != '') {
                var opt1 = document.createElement("option");
                document.getElementById("DropDownList_CustomDocumentTransfers_ReceivingProgram").options.add(opt1);
                opt1.text = ''; //
                opt1.value = -1;
                $.xmlDOM(result.xml).find("StaffProgram").each(function() {
                    //  $("<option value=" + this.childNodes[1].text + ">" + this.childNodes[0].text + "</option>").appendTo("[id$=DropDownList_Staff_DefaultMultiStaffViewId]");
                     ...
                     var opt = document.createElement("option");
                    document.getElementById("DropDownList_CustomDocumentTransfers_ReceivingProgram").options.add(opt);
    ...
    function ValidateCustomPageEventHandler() {
        ...
        if ($("#DropDownList_CustomDocumentTransfers_ReceivingStaff").val() == '-1') {
            ShowHideErrorMessage("Please select Receiving Staff.", 'true');
            return false;
    }

    if ($("#DropDownList_CustomDocumentTransfers_ReceivingProgram").val() == '-1') {
