...
function ValidateCustomPageEventHandler() {
    ...
    if ($("#RadioButton_CustomDocumentDiagnosticAssessments_TypeOfAssessment_A").length != 0 && $("#RadioButton_CustomDocumentDiagnosticAssessments_TypeOfAssessment_M").length != 0 && $("#RadioButton_CustomDocumentDiagnosticAssessments_TypeOfAssessment_E").length != 0 && $("#RadioButton_CustomDocumentDiagnosticAssessments_TypeOfAssessment_C").length != 0) {
        if (!(AssessmentTypeA || AssessmentTypeM || AssessmentTypeE || AssessmentTypeC)) {
            ShowHideErrorMessage('Please select atleast one option from Assessment Type ', 'true');
            return false;
        }
    }
...
function SetScreenSpecificValues(dom, action) {
     try {
        // Following changes have been done to assign the tabobject with valid object of RadTabStrip
        // sometimes the StoreTabstripClientObject() takes times to execute and thus tabobject is not initialized.
        // To rectify this behavior, call to SetScreenSpecificValues() is being made again with some delay so that tabobject is initialized. 
...
function AddEventHandlers() {
    try {
        ...
        //------------Modified click events to change event to prevent autosave xml creation based on validation--------
        $("#RadioButton_CustomDocumentDiagnosticAssessments_TypeOfAssessment_A").bind("change", function(e) {
            if (!SetTabsAssessmentType()) {
                e.stopImmediatePropagation();
                return false;
            }
        });
        $("#RadioButton_CustomDocumentDiagnosticAssessments_TypeOfAssessment_M").bind("change", function(e) {
            if (!SetTabsAssessmentType()) {
                e.stopImmediatePropagation();
                return false;
            }
        });
        $("#RadioButton_CustomDocumentDiagnosticAssessments_TypeOfAssessment_E").bind("change", function(e) {
            if (!SetTabsAssessmentType()) {
                e.stopImmediatePropagation();
                return false;
            }
        });
        $("#RadioButton_CustomDocumentDiagnosticAssessments_TypeOfAssessment_C").bind("change", function(e) {
            if (!SetTabsAssessmentType()) {
                e.stopImmediatePropagation();
                return false;
            }
        });
        $("[id$=DropDownList_CustomDocumentDiagnosticAssessments_LevelofCare]").bind("change", function(e) {
            if (!SetTabsAssessmentType()) {
                $(this).val("");
                e.stopImmediatePropagation();
                return false;
        ...
        //------------Modified click events to change event ends--------
    }
    catch (err) {
