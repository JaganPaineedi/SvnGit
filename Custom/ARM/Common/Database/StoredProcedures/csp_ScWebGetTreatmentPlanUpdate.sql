/****** Object:  StoredProcedure [dbo].[csp_ScWebGetTreatmentPlanUpdate]    Script Date: 08/27/2015 15:34:16 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_ScWebGetTreatmentPlanUpdate]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_ScWebGetTreatmentPlanUpdate]
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[csp_ScWebGetTreatmentPlanUpdate]           
...
-- =============================================          
/*********************************************************************/                                                                              
/* Stored Procedure: [csp_ScWebGetTreatmentPlanUpdate]     */                                                                     
...
/* Feb 26, 2018    Vijeta        Converted ssp_ScWebGetTreatmentPlanUpdate to csp_ScWebGetTreatmentPlanUpdate for 	A Renewed Mind - Support #814     */  
/*********************************************************************/          
 -- CustomTreatmentPlan           
...
set @CurrentMentalHealthDiagnoses ='Effective Date:'+ @EffectiveDate    
 ...
 --CustomTPNeeds        
   DECLARE @DocumentVersionIdNeed int    
     set @DocumentVersionIdNeed= (SELECT TOP (1) CurrentDocumentVersionId FROM Documents where ClientId=@ClientID AND Status =22 AND DocumentCodeId in (1483,1484,1485) AND ISNULL(RecordDeleted, 'N') = 'N' order by ModifiedDate desc)                     
  ...
  SELECT Distinct            
    ...
    AND CTG.DocumentVersionId in (@DocumentVersionIdNeed,@DocumentVersionId)
...
BEGIN CATCH        
 ...
 SET @Error= Convert(varchar,ERROR_NUMBER()) + '*****' + Convert(varchar(4000),ERROR_MESSAGE())                                                                                     
    + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'csp_ScWebGetTreatmentPlanUpdate')                                                                                     
