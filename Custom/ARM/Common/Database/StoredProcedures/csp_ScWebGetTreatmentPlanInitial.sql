/****** Object:  StoredProcedure [dbo].[csp_ScWebGetTreatmentPlanInitial]    Script Date: 08/20/2015 13:04:38 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_ScWebGetTreatmentPlanInitial]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_ScWebGetTreatmentPlanInitial]
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[csp_ScWebGetTreatmentPlanInitial] --220,550              
...
-- Feb 26, 2018    Vijeta        Converted ssp_ScWebGetTreatmentPlanInitial to csp_ScWebGetTreatmentPlanInitial for 	A Renewed Mind - Support #814                                                                      
/*********************************************************************/            
 -- CustomTreatmentPlan               
...
---------------CustomTPNeeds      
   DECLARE @DocumentVersionIdNeed int    
     set @DocumentVersionIdNeed= (SELECT TOP (1) CurrentDocumentVersionId FROM Documents where ClientId=@ClientID AND Status =22 AND DocumentCodeId in (1483,1484,1485,1486) AND ISNULL(RecordDeleted, 'N') = 'N' order by ModifiedDate desc)                     
...
BEGIN CATCH            
 ...
 SET @Error= Convert(varchar,ERROR_NUMBER()) + '*****' + Convert(varchar(4000),ERROR_MESSAGE())              
    + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'csp_ScWebGetTreatmentPlanInitial')                                                                                         
