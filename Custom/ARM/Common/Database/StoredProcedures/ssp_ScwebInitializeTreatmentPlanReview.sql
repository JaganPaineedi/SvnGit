/****** Object:  StoredProcedure [dbo].[ssp_ScwebInitializeTreatmentPlanReview]    Script Date: 08/27/2015 15:00:24 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_ScwebInitializeTreatmentPlanReview]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_ScwebInitializeTreatmentPlanReview]
GO
...
GO
 ...
 CREATE procedure [dbo].[ssp_ScwebInitializeTreatmentPlanReview] --3,550,N''  
...
BEGIN  
/*********************************************************************/  
/* Stored Procedure: [ssp_ScwebInitializeTreatmentPlanReview]     */  
...
----------CustomTPNeed  
  ...
  SELECT  
    'CustomTPNeeds' AS TableName  
      ...
      ,(Case when (select count(1) from CustomTPGoalNeeds where NeedId =CustomTPNeeds.NeedId and isnull(CustomTPGoalNeeds.RecordDeleted ,'N')<>'Y')>=1 then 'Y' else 'N' end) as LinkedInSession  
    FROM CustomTPNeeds  
...
BEGIN CATCH  
 ...
 SET @Error= Convert(varchar,ERROR_NUMBER()) + '*****' + Convert(varchar(4000),ERROR_MESSAGE())  
    + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'ssp_ScwebInitializeTreatmentPlanReview')  
