...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_RDLCustomCurrentVitalsPsycNote]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_RDLCustomCurrentVitalsPsycNote] 
GO

/****** Object:  StoredProcedure [dbo].[csp_RDLCustomCurrentVitalsPsycNote]    Script Date: 06/30/2014 18:06:43 ******/
SET ANSI_NULLS ON
GO
...
Create PROCEDURE [dbo].[csp_RDLCustomCurrentVitalsPsycNote]  (@DocumentVersionId INT)   
  AS    
  /*********************************************************************/
/* Stored Procedure: [csp_RDLCustomCurrentVitalsPsycNote] 737170    */
/*       Date              Author                  Purpose                   */
...
CREATE TABLE #Results(id int identity(1,1), vitals varchar(max),value varchar(500),Color varchar(500))

INSERT INTO #Results(vitals,value,Color)
 --Height                                    
    SELECT   attributename , 
			ISNULL(CASE WHEN datatype = 8081 THEN dbo.Getglobalcodename(value)ELSE value END, '')
...
INSERT INTO #Results(vitals,value,Color)  
  ...
  INSERT INTO #Results(vitals,value,Color)                           
      ...
      ELSE   
       ...
       SELECT   attributename , ISNULL(CASE WHEN datatype = 8081 THEN dbo.Getglobalcodename(value)        
        ...
        FROM    #tempvitals t
        LEFT JOIN #tempvitalsColors ta On t.healthdataattributeid=ta.healthdataattributeid
        WHERE  healthdatasubtemplateid=114 and t.healthdataattributeid=115        
        ORDER BY attributename 
    ...
    INSERT INTO #Results(vitals,value,Color)  
        ...
        FROM    #tempvitals t
        LEFT JOIN #tempvitalsColors ta On t.healthdataattributeid=ta.healthdataattributeid
        WHERE  t.healthdatasubtemplateid=112 and t.healthdataattributeid=119        
        ORDER BY attributename 
      ...
      INSERT INTO #Results(vitals,value,Color)  
            ...
            FROM    #tempvitals t
            LEFT JOIN #tempvitalsColors ta On t.healthdataattributeid=ta.healthdataattributeid
            WHERE  t.healthdatasubtemplateid=117 and t.healthdataattributeid=123        
            ORDER BY attributename  
   ...
   INSERT INTO #Results(vitals,value,Color)  
     ...
     INSERT INTO #Results(vitals,value,Color)  
        ...
        FROM    #tempvitals t
        LEFT JOIN #tempvitalsColors ta On t.healthdataattributeid=ta.healthdataattributeid
        WHERE  t.healthdatasubtemplateid=115 and t.healthdataattributeid=124        
        ORDER BY attributename 
    ...
    SELECT vitals as currentvitals,value,Color FROM    #Results
 ...
 BEGIN CATCH    
  ...
  SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'csp_RDLCustomCurrentVitalsPsycNote') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())    
