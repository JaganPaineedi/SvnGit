...
/****** Object:  StoredProcedure [dbo].[csp_RdlSubReportPsychiatricNoteSubjective]    Script Date: 07/14/2015 15:09:05 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_RdlSubReportPsychiatricNoteSubjective]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_RdlSubReportPsychiatricNoteSubjective] 
GO
...
CREATE PROCEDURE [dbo].[csp_RdlSubReportPsychiatricNoteSubjective] 
@DocumentVersionId INT  
AS  
/*********************************************************************/  
/* Stored Procedure: [csp_RdlSubReportPsychiatricNoteSubjective]   */  
/*       Date              Author                  Purpose                   */  
/*      07-14-2014      Lakshmi Kanth             To Retrieve Data  for RDL   */  
/*********************************************************************/  
BEGIN  
 BEGIN TRY  
    SELECT   
  ...
  GC8.CodeName as Duration, --TimeOfDayAllday 
  ...
  CP.LocationOtherText as LocationOtherText,  
  CP.ContextText,   
 ...
 BEGIN CATCH  
  ...
  SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'csp_RdlSubReportPsychiatricNoteSubjective') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())  
