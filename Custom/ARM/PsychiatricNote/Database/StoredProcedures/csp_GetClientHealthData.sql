 /****** Object:  StoredProcedure [dbo].[csp_GetClientHealthData]    Script Date: 07/07/2015 16:26:11 ******/
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[csp_GetClientHealthData]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[csp_GetClientHealthData]
GO
...
GO
 ...
 CREATE PROCEDURE [dbo].[csp_GetClientHealthData] (  
 @ClientId INT  
 ...
 /********************************************************************************                                                                          
-- Stored Procedure: dbo.csp_GetClientHealthData.sql                                                                            
--   csp_GetClientHealthData 4,110                                                                       
...
BEGIN  
 ...
 AND ISNULL(HDST.RecordDeleted, 'N') = CASE   
  ...
  --  Count(CHDA.HealthDataAttributeId) as CountRows 
  -- ,CHDA.HealthDataSubTemplateId  
  --FROM ClientHealthDataAttributes CHDA  
  --INNER JOIN #FilteredAttributes FA ON FA.HealthDataAttributeId = CHDA.HealthDataAttributeId  
  --INNER JOIN HealthDataAttributes HDA ON HDa.HealthDataAttributeId = FA.HealthDataAttributeId  
  ...
  --  ELSE 'N'  
  --  END 
  --  GROUP BY CHDA.HealthDataSubTemplateId  
 ...
 BEGIN CATCH  
  ...
  SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'csp_GetClientHealthData.sql') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + Convert(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())  
