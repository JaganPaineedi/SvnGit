...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_RDLCustomPreviousVitalsPsycNote]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_RDLCustomPreviousVitalsPsycNote] 
GO

/****** Object:  StoredProcedure [dbo].[csp_RDLCustomPreviousVitalsPsycNote]    Script Date: 06/30/2014 18:06:43 ******/
SET ANSI_NULLS ON
GO	
...
Create PROCEDURE [dbo].[csp_RDLCustomPreviousVitalsPsycNote]  (@DocumentVersionId INT)   
  AS    
  /*********************************************************************/
/* Stored Procedure: [csp_RDLCustomPreviousVitalsPsycNote] 737170
  */
/*       Date              Author                  Purpose                   */
...
,	NAME  from distincttemp where row=1
 ...
 BEGIN CATCH    
  ...
  SET @Error = CONVERT(VARCHAR, ERROR_NUMBER()) + '*****' + CONVERT(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + ISNULL(CONVERT(VARCHAR, ERROR_PROCEDURE()), 'csp_RDLCustomPreviousVitalsPsycNote') + '*****' + CONVERT(VARCHAR, ERROR_LINE()) + '*****' + CONVERT(VARCHAR, ERROR_SEVERITY()) + '*****' + CONVERT(VARCHAR, ERROR_STATE())    
