IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'[dbo].[csp_PMClaims837ProfessionalMITS]')
...
-- 12/2/2016	MJensen		Added separator to SV1 segment & changed the default for EmergencyIndicator to ''.  						
*********************************************************************************/  
as  
        ...
        INSERT  INTO #ServiceDiagnosis
            ...
            FROM    ( SELECT DISTINCT
                    ...
                    -- I noticed the asterisk character getting saved in ServiceDiagnosis.ICD10 so removing it here
                                CASE WHEN DATEDIFF(day, @ICD10StartDate, a.DateofService) >= 0 THEN REPLACE(ICD10Code, '*', '')
                                     ELSE ISNULL(b.DSMCode, b.ICD9Code)
                                END AS DiagnosisCode
                FROM    #Services a
                        JOIN dbo.ServiceDiagnosis b ON b.ServiceId = a.ServiceId AND ISNULL(b.RecordDeleted, 'N') = 'N'
...
-- Coalesce(@tse_sep + EmergencyIndicator ,'') 
 + @te_sep +  
 -- 12/2/2016 mjensen added separator - count is wrong
 @te_sep +
 -- 12/2/2016 mjensen changed emergency indicator default from 'N' to ''
/*SV109*/CASE WHEN Coalesce(EmergencyIndicator,'') = '' THEN '' ELSE 'Y' END  
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,  
