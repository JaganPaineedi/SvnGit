/****** Object:  StoredProcedure [dbo].[csp_PMClaims837ProfessionalOfficeAlly]    Script Date: 01/15/2015 15:33:40 ******/
IF EXISTS ( SELECT  *
            FROM    sys.objects
            WHERE   object_id = OBJECT_ID(N'[dbo].[csp_PMClaims837ProfessionalOfficeAlly]')
                    AND type IN ( N'P', N'PC' ) )
    DROP PROCEDURE [dbo].[csp_PMClaims837ProfessionalOfficeAlly]
GO
...
CREATE PROCEDURE [dbo].[csp_PMClaims837ProfessionalOfficeAlly]
    @CurrentUser VARCHAR(30) ,
    @ClaimBatchId INT    
/********************************************************************************    
-- Stored Procedure: dbo.csp_PMClaims837ProfessionalOfficeAlly     
--    
-- Copyright: 2006 Streamline Healthcate Solutions    
--    
-- Purpose: Generates electronic claims for Commcercial clients to be submitted to Office Ally
--
-- Based on ssp_PMClaims837ProfessionalVersion5  
-- 
-- Created on 9/30/2013: Changed Receiver and Sender Qualifiers from ZZ to 30, based on the Office Ally Companion Guide
--    
-- Updates:                                                           
-- Date        Author      Purpose    
...
-- 10/01/2013   NJain       Updated for Office Ally clearing house: 
                            - Sender and Receiver Qualifiers in the HIPPA HeaderTrailer = 30, Respective Tax IDs	
                            - Changed the name in 837 HeaderTrailer from BCBSM to OfficeAlly
                            - Updated 837HeaderTrailer to be 5010 instead of 4010
...
RevenueCodeDescription = b.RevenueCodeDescription,   ClaimUnits = b.ClaimUnits    
from #ClaimLines a     
...
-- Populate Header and Trailer #837HeaderTrailer    
            ...
            INSERT  INTO #837HeaderTrailer
                    ...
                    SELECT  REPLICATE('0', 8 - LEN(CONVERT(VARCHAR, @ClaimBatchId))) + CONVERT(VARCHAR, @ClaimBatchId) + CONVERT(VARCHAR, @BatchFileNumber - 1) ,
                            ...
                            SUBSTRING(CONVERT(VARCHAR, GETDATE(), 108), 1, 2) + SUBSTRING(CONVERT(VARCHAR, GETDATE(), 108), 4, 2) ,
                            'CH' ,
                            ISNULL(a.Version, '005010x222') , --case when a.ProductionOrTest = 'T' then '004010X098DA1' else '004010X098A1' end,    
                            '2' ,
                            b.AgencyName ,
...
FacilitySecondaryQualifier2 ,FacilitySecondaryId2*/    
                    ...
                    SELECT  b.UniqueId ,
                            ...
                            MAX(CASE WHEN CHARINDEX('.', DiagnosisCode1) > 0 THEN SUBSTRING(DiagnosisCode1, 1, 3) + SUBSTRING(DiagnosisCode1, 5, LEN(DiagnosisCode1) - 4)
                                     ELSE DiagnosisCode1
                                END) ,
                            MAX(CASE WHEN CHARINDEX('.', DiagnosisCode2) > 0 THEN SUBSTRING(DiagnosisCode2, 1, 3) + SUBSTRING(DiagnosisCode2, 5, LEN(DiagnosisCode2) - 4)
                                     ELSE DiagnosisCode2
                                END) ,
                            MAX(CASE WHEN CHARINDEX('.', DiagnosisCode3) > 0 THEN SUBSTRING(DiagnosisCode3, 1, 3) + SUBSTRING(DiagnosisCode3, 5, LEN(DiagnosisCode3) - 4)
                                     ELSE DiagnosisCode3
                                END) ,
                            MAX(CASE WHEN CHARINDEX('.', DiagnosisCode4) > 0 THEN SUBSTRING(DiagnosisCode4, 1, 3) + SUBSTRING(DiagnosisCode4, 5, LEN(DiagnosisCode4) - 4)
                                     ELSE DiagnosisCode4
                                END) ,
                            MAX(CASE WHEN CHARINDEX('.', DiagnosisCode5) > 0 THEN SUBSTRING(DiagnosisCode5, 1, 3) + SUBSTRING(DiagnosisCode5, 5, LEN(DiagnosisCode5) - 4)
                                     ELSE DiagnosisCode5
                                END) ,
                            MAX(CASE WHEN CHARINDEX('.', DiagnosisCode6) > 0 THEN SUBSTRING(DiagnosisCode6, 1, 3) + SUBSTRING(DiagnosisCode6, 5, LEN(DiagnosisCode6) - 4)
                                     ELSE DiagnosisCode6
                                END) ,
                            MAX(CASE WHEN CHARINDEX('.', DiagnosisCode7) > 0 THEN SUBSTRING(DiagnosisCode7, 1, 3) + SUBSTRING(DiagnosisCode7, 5, LEN(DiagnosisCode7) - 4)
                                     ELSE DiagnosisCode7
                                END) ,
                            MAX(CASE WHEN CHARINDEX('.', DiagnosisCode8) > 0 THEN SUBSTRING(DiagnosisCode8, 1, 3) + SUBSTRING(DiagnosisCode8, 5, LEN(DiagnosisCode8) - 4)
                                     ELSE DiagnosisCode8
                                END) ,
                            '82' ,
                            '1' ,
...
--Update segment information for #837Claims    
            ...
            SET     CLMSegment = UPPER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(( 'CLM' + @te_sep + ClaimId + @te_sep + ( CASE WHEN CONVERT(INT, TotalAmount * 100) = CONVERT(INT, TotalAmount) * 100 THEN CONVERT(VARCHAR, CONVERT(INT, TotalAmount))
                    ...
                    PayerClaimControlNumberRefSegment = CASE WHEN ISNULL(RTRIM(PayerClaimControlNumber), '') = '' THEN NULL
                                                        ...
                                                        END ,
                    HISegment = UPPER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(( 'HI' + @te_sep + 'BK' + @tse_sep + DiagnosisCode1 + ( CASE WHEN DiagnosisCode2 IS NOT NULL THEN @te_sep + 'BF' + @tse_sep + DiagnosisCode2 + ( CASE WHEN DiagnosisCode3 IS NOT NULL THEN @te_sep + 'BF' + @tse_sep + DiagnosisCode3 + ( CASE WHEN DiagnosisCode4 IS NOT NULL THEN @te_sep + 'BF' + @tse_sep + DiagnosisCode4 + ( CASE WHEN DiagnosisCode5 IS NOT NULL THEN @te_sep + 'BF' + @tse_sep + DiagnosisCode5 + ( CASE WHEN DiagnosisCode6 IS NOT NULL THEN @te_sep + 'BF' + @tse_sep + DiagnosisCode6 + ( CASE WHEN DiagnosisCode7 IS NOT NULL THEN @te_sep + 'BF' + @tse_sep + DiagnosisCode7 + ( CASE WHEN DiagnosisCode8 IS NOT NULL THEN @te_sep + 'BF' + @tse_sep + DiagnosisCode8
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         ELSE RTRIM('')
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    END )
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ELSE RTRIM('')
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           END )
                    ...
                    FacilityNM1Segment = CASE WHEN ISNULL(RTRIM(FacilityAddress1), '') = ''
                                              ...
                                              ELSE UPPER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(( 'NM1' + @te_sep + FacilityEntityCode + @te_sep + '2' + @te_sep + FacilityName + @te_sep + @te_sep + @te_sep + @te_sep + @te_sep + FacilityidQualifier + @te_sep + FacilityId ), @e_sep, ''), @se_sep, ''), @seg_term, ''), '&', ''), @te_sep, @e_sep), @tse_sep, @se_sep)) + @seg_term
                                         END ,
...
InsuredSecondaryId     
                    ...
                    PayerPaymentDTPSegment = NULL --srf 3/22/2011 set to blank per version 5 guide --   The Claim Check or Remittance Date (Loop 2330, DTP) is only required when the Line Adjudication Information (Loop 2430, SVD) is not used and the claim has been previousl
...
-- Other Insured Adjustment Information    
            UPDATE  a
            SET     SVDSegment = UPPER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(( 'SVD' + @te_sep + a.PayerId + @te_sep + ( CASE WHEN CONVERT(INT, a.PayerPaidAmount * 100) = CONVERT(INT, a.PayerPaidAmount) * 100 THEN CONVERT(VARCHAR, CONVERT(INT, a.PayerPaidAmount))
                                                                                                                                                                                                    ...
                                                                                                                                                                                                    END + CASE WHEN e.HIPAACode4 IS NULL
                                                                                                                                                                                                          ...
                                                                                                                                                                                                          END + CASE WHEN e.HIPAACode5 IS NULL
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                END + CASE WHEN e.HIPAACode6 IS NULL
                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                      END ), @e_sep, ''), @se_sep, ''), @seg_term, ''), '&', ''), @te_sep, @e_sep), @tse_sep, @se_sep)) + @seg_term
                                  END ,
                    ServiceAdjudicationDTPSegment = CASE WHEN ISNULL(RTRIM(a.PaymentDate), '') = '' THEN NULL
                                                         ELSE UPPER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(( 'DTP' + @te_sep + '573' + @te_sep + 'D8' + @te_sep + a.PaymentDate ), @e_sep, ''), @se_sep, ''), @seg_term, ''), '&', ''), @te_sep, @e_sep), @tse_sep, @se_sep)) + @seg_term
                                                    END
...
--OIN4Segment, OIRefSegment, PayerNM1Segment, PayerPaymentDTPSegment, PayerRefSegment, --srf 1/19/2012 added PayerRefSegment    
--AuthorizationNumberRefSegment    
--from #837OtherInsureds    
