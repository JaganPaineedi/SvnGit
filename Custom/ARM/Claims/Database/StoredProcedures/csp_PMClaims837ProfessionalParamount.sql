/****** Object:  StoredProcedure [dbo].[csp_PMClaims837ProfessionalParamount]    Script Date: 01/21/2015 11:13:56 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_PMClaims837ProfessionalParamount]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_PMClaims837ProfessionalParamount]
GO
...
CREATE procedure [dbo].[csp_PMClaims837ProfessionalParamount]
@CurrentUser varchar(30), 
@ClaimBatchId int
/********************************************************************************
-- Stored Procedure: dbo.[csp_PMClaims837ProfessionalParamount] 
--
...
-- 06.14.2012  JJN		   Altered to fit Paramount claims
   07.13.2012  avoss	   added diagnoses codes to #Charges
   01.02.2013  s.stevens   added clinician degree code to separate RN charges from clinicians with other degrees.
...
CREATE TABLE #Charges(
	...
	ClinicianDegreeCode varchar(250) null,	
)
...
'PARAMOUNT', --case when g.electronicclaimspayerid = 'MACSIS' then 'MACSIS' else g.CoveragePlanName end, --srf 1/12/2012 set coverageplanname to MACSIS per MACSIS 
...
chg.ChargeAmount, c.ReferringId
...
-- Updat redering provider degree code name - spp-s
update a
set ClinicianDegreeCode = (case c.codename when 'RN' then 'RN' else '' end)
from #Charges a
JOIN staff b ON (a.ClinicianId = b.StaffId)
JOIN GlobalCodes c  ON (b.Degree = c.GlobalCodeId)
...
ChargeAmount, ChargeId, SubmissionReasonCode, ClinicianDegreeCode)
...
ProcedureCodeId, convert(datetime, convert(varchar,DateOfService,101)), 
ClientCoveragePlanId, PlaceOfService, ClinicianDegreeCode
...
and isnull(a.ClinicianDegreeCode,'') = isnull(b.ClinicianDegreeCode,'')
)
...
insert into #OtherInsuredAdjustment
(OtherInsuredId, ARLedgerId, HIPAACode,HipaaGroupCode, LedgerType, Amount)
select a.OtherInsuredId, null, CASE WHEN f.ExternalCode1 IS NULL THEN '45' ELSE f.ExternalCode1 END, 
	CASE WHEN f.ExternalCode2 = 'TRANS' THEN 'PR' ELSE 'CO' END, 4204, -b.ChargeAmount
from #OtherInsured a
JOIN #Charges b  ON (a.ClaimLineId = b.ClaimLineId)
JOIN Charges d ON (b.ServiceId = d.ServiceId
and d.ClientCoveragePlanId = a.ClientCoveragePlanId)
...
-- Update #ClaimLines
        UPDATE  a
        SET     DiagnosisCode1 = b.DiagnosisCode ,
                DiagnosisPointer1 = '1'
        FROM    #ClaimLines a
                JOIN #ServiceDiagnosis b ON b.ClaimLineId = a.ClaimLineId
        WHERE   b.Rnk = 1
        ...
        UPDATE  a
        SET     DiagnosisCode5 = b.DiagnosisCode ,
                DiagnosisPointer5 = '5'
        FROM    #ClaimLines a
                JOIN #ServiceDiagnosis b ON b.ClaimLineId = a.ClaimLineId
        WHERE   b.Rnk = 5
        ...
        UPDATE  a
        SET     DiagnosisCode6 = b.DiagnosisCode ,
                DiagnosisPointer6 = '6'
        FROM    #ClaimLines a
                JOIN #ServiceDiagnosis b ON b.ClaimLineId = a.ClaimLineId
        WHERE   b.Rnk = 6
        ...
        UPDATE  a
        SET     DiagnosisCode7 = b.DiagnosisCode ,
                DiagnosisPointer7 = '7'
        FROM    #ClaimLines a
                JOIN #ServiceDiagnosis b ON b.ClaimLineId = a.ClaimLineId
        WHERE   b.Rnk = 7
        ...
        UPDATE  a
        SET     DiagnosisCode8 = b.DiagnosisCode ,
                DiagnosisPointer8 = '8'
        FROM    #ClaimLines a
                JOIN #ServiceDiagnosis b ON b.ClaimLineId = a.ClaimLineId
        WHERE   b.Rnk = 8
...
NumberOfTransactions,		GroupControlNumberTrailer,		NumberOfGroups,		InterchangeControlNumberTrailer 
)
select	'00',	space(10),		'00',		space(10), 
'ZZ',	LEFT(BillingLocationCode + SPACE(15),15),'ZZ', 
LEFT(ReceiverCode + 'HEALTH' + SPACE(15) ,15) ,right(convert(varchar,getdate(),112),6), substring(convert(varchar,getdate(),108),1,2) + substring(convert(varchar,getdate(),108),4,2),  
'^','00501', 
replicate('0',9-len(convert(varchar,@ClaimBatchId))) + convert(varchar,@ClaimBatchId), '0', 
ProductionOrTest, @tse_sep , 'HC', BillingLocationCode + 'T837P',
ReceiverCode+'837P', convert(varchar,getdate(),112), substring(convert(varchar,getdate(),108),1,2) + substring(convert(varchar,getdate(),108),4,2),
convert(varchar,@ClaimBatchId), 'X', '005010X222A1', 
'1', convert(varchar,@ClaimBatchId), '1', replicate('0',9-len(convert(varchar,@ClaimBatchId))) + convert(varchar,@ClaimBatchId)
from ClaimFormats
...
/*GS00*/'GS' + @te_sep + 
/*GS01*/Case When FunctionalIdCode in ('HS','HR','HI','RA','BE','HC') Then FunctionalIdCode Else NULL END+ @te_sep + 
...
/*GE00*/'GE' + @te_sep + 
/*GE00*/NumberOfTransactions  + @te_sep + 
/*GE00*/GroupControlNumberTrailer  
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
...
'LUCAS COUNTY MHRSB', a.ReceiverPrimaryId, 
...
max(b.AgencyName),max(b.Address),max(b.City), max(b.State), max(b.ZipCode),
...
SubscriberDOB , SubscriberSex , SubscriberSSN, PayerName ,PayerIdQualifier ,
...
max(rtrim(a.InsuredZip)) , max(convert(varchar,a.InsuredDOB,112)) , 
...
max(a.PlaceOfServiceCode), max(a.SubmissionReasonCode), convert(varchar,max(a.RelatedHospitalAdmitDate),112), 'Y','A','Y','I','',  -- srf 3/14/2011 -- 'I' and '' set per instructions.
...
'XX',--max(case when a.RenderingProviderNPI is null then a.RenderingProviderTaxIdType else 'XX' end),
...
max(a.FacilityProviderIdType), max(a.FacilityProviderId)
/*,
...
'ST' + @te_sep + '837' + @te_sep + TransactionSetControlNumberHeader + @te_sep + ImplementationConventionReference 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
BHTSegment = UPPER(replace(replace(replace(replace(replace(replace((
'BHT' + @te_sep + '0019' + @te_sep + TransactionSetPurposeCode + @te_sep + 
ApplicationTransactionId + @te_sep + CreationDate + @te_sep + CreationTime + @te_sep + EncounterId
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
...
'NM1' + @te_sep + '40' + @te_sep + '2' + @te_sep + 
'HARRBOR'-- ReceiverLastName
 + @te_sep + @te_sep + @te_sep + @te_sep + @te_sep + 
'46' + @te_sep + ReceiverPrimaryId  
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term
...
case when isnull(rtrim(BillingProviderFirstName),'') = '' then '2' else '1' end + 
@te_sep + 
BillingProviderLastName + @te_sep + 
case when isnull(rtrim(BillingProviderFirstName),'') = '' then rtrim('') else rtrim(BillingProviderFirstName) end + @te_sep + 
case when isnull(rtrim(BillingProviderMiddleName),'') = '' then rtrim('') else rtrim(BillingProviderMiddleName) end + @te_sep + 
@te_sep + 
case when isnull(rtrim(BillingProviderSuffix),'') = '' then rtrim('') else rtrim(BillingProviderSuffix) end + @te_sep + 
BillingProvideridQualifier + @te_sep + BillingProviderid 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
...
BillingProviderRefSegment = NULL,--case when BillingProviderAdditionalId is null then null else
--UPPER(replace(replace(replace(replace(replace(replace((
--'REF' + @te_sep + BillingProviderAdditionalIdQualifier  + @te_sep + 
--BillingProviderAdditionalId 
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
BillingProviderRef2Segment = case when BillingProviderAdditionalId2 is null then null else
UPPER(replace(replace(replace(replace(replace(replace((
'REF' + @te_sep + BillingProviderAdditionalIdQualifier2  + @te_sep + 
BillingProviderAdditionalId2 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
BillingProviderRef3Segment = NULL,--case when BillingProviderAdditionalId3 is null then null else
--UPPER(replace(replace(replace(replace(replace(replace((
...
'NM1' + @te_sep + '87' + @te_sep + '2' + @te_sep + 
PayToProviderLastName + @te_sep + @te_sep + @te_sep + @te_sep + @te_sep + 
...
'SBR' + @te_sep + (case Priority when 1 then 'P' when 2 then 'S' else 'T' end) + @te_sep + 
case when RelationCode = '18' then '18' else rtrim('') end + 
case when isnull(rtrim(GroupNumber),'') = '' then @te_sep 
else @te_sep + rtrim(GroupNumber) end +
case when isnull(rtrim(GroupName),'') = '' or isnull(rtrim(GroupNumber),'') <> '' then @te_sep 
else @te_sep + rtrim(GroupName) end +
case when isnull(rtrim(MedicareInsuranceTypeCode),'') = '' then @te_sep 
else @te_sep + rtrim(MedicareInsuranceTypeCode) end +
@te_sep + @te_sep + @te_sep + @te_sep + 
rtrim(ClaimFilingIndicatorCode)
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term, 
SubscriberNM1Segment = UPPER(replace(replace(replace(replace(replace(replace((
'NM1' + @te_sep + 'IL' + @te_sep + SubscriberEntityQualifier + @te_sep + 
SubscriberLastName + @te_sep + 
(case when isnull(rtrim(SubscriberFirstName),'') = '' then rtrim('') else SubscriberFirstName end)
 + @te_sep + 
(case when isnull(rtrim(SubscriberMiddleName),'') = '' then rtrim('') else SubscriberMiddleName end) 
 + @te_sep + 
(case when isnull(rtrim(SubscriberSuffix),'') = '' then @te_sep else @te_sep + rtrim(SubscriberSuffix) end) 
+ @te_sep + SubscriberIdQualifier + @te_sep +  replace(replace(InsuredId,'-',rtrim('')),' ',rtrim(''))
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term, 
SubscriberN3Segment = CASE WHEN RelationCode = '18' THEN 
UPPER(replace(replace(replace(replace(replace(replace((
'N3' + @te_sep + rtrim(replace(replace(replace(SubscriberAddress1,'#',' '),'.',' '),'-',' '))  +
...
SubscriberN4Segment = case when RelationCode = '18' then 
UPPER(replace(replace(replace(replace(replace(replace((
'N4' + @te_sep + SubscriberCity + @te_sep + SubscriberState + @te_sep + 
SubscriberZip 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term else null end,
SubscriberDMGSegment = case when RelationCode = '18' then 
UPPER(replace(replace(replace(replace(replace(replace((
'DMG' + @te_sep + 'D8' + @te_sep + SubscriberDOB + @te_sep + 
(case when SubscriberSex is null then 'U' else SubscriberSex end) 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term else null end, 
SubscriberRefSegment = NULL,--case when SubscriberSSN is null then null else
--UPPER(replace(replace(replace(replace(replace(replace((
--'REF' + @te_sep + 'SY' + @te_sep + SubscriberSSN
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
PayerNM1Segment = UPPER(replace(replace(replace(replace(replace(replace((
'NM1' + @te_sep + 'PR' + @te_sep + '2' + @te_sep +  
PayerName + @te_sep + @te_sep + @te_sep + @te_sep + @te_sep + 
PayerIdQualifier + @te_sep + ElectronicClaimsPayerId 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
PayerN3Segment = case when PayerAddress1 is null then null else
UPPER(replace(replace(replace(replace(replace(replace((
...
PayerN4Segment = case when PayerAddress1 is null then null else
UPPER(replace(replace(replace(replace(replace(replace((
...
/*
PARAMOUNT IS ALWAYS SEND '18', Self as subscriber. This logic should be corrected to put the Patient Information in even if '18' is the relationship code and 
the paitient is not the subscriber
*/
PatientPatSegment = case when RelationCode IN ('18','34') then null else
UPPER(replace(replace(replace(replace(replace(replace((
'PAT' + @te_sep + RelationCode
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end, 
PatientNM1Segment = 
/*
PARAMOUNT IS ALWAYS SEND '18', Self as subscriber. This logic should be corrected to put the Patient Information in even if '18' is the relationship code and 
the paitient is not the subscriber
*/
case when RelationCode IN ('18','34') then null else
UPPER(replace(replace(replace(replace(replace(replace((
...
PatientN3Segment = case when RelationCode IN ('18','34') then null else
UPPER(replace(replace(replace(replace(replace(replace((
...
PatientN4Segment = case when RelationCode IN ('18','34') then null else
UPPER(replace(replace(replace(replace(replace(replace((
'N4' + @te_sep + ClientCity + @te_sep + ClientState + @te_sep + 
ClientZip 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
PatientDMGSegment = case when RelationCode IN ('18','34') then null else
UPPER(replace(replace(replace(replace(replace(replace((
...
ClaimId +
@te_sep + 
(case when convert(int,TotalAmount*100) = convert(int,TotalAmount)*100 then 
convert(varchar,convert(int,TotalAmount)) else
convert(varchar,TotalAmount) end) + @te_sep + @te_sep + @te_sep + 
PlaceOfService + @tse_sep + 'B' + @tse_sep + SubmissionReasonCode + @te_sep + 
SignatureIndicator + @te_sep + MedicareAssignCode + @te_sep + 
BenefitsAssignCertificationIndicator + @te_sep + ReleaseCode + @te_sep + PatientSignatureCode  
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
AdmissionDateDTPSegment = case when RelatedHospitalAdmitDate is null then null else
UPPER(replace(replace(replace(replace(replace(replace((
...
UPPER(replace(replace(replace(replace(replace(replace((
'REF' + @te_sep + 'G1' + @te_sep + PriorAuthorizationNumber
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
PayerClaimControlNumberRefSegment = case when isnull(rtrim(PayerClaimControlNumber),'') = '' then null else
UPPER(replace(replace(replace(replace(replace(replace((
'REF' + @te_sep + 'F8' + @te_sep + PayerClaimControlNumber
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
HISegment = UPPER(replace(replace(replace(replace(replace(replace((
'HI' + @te_sep + 'BK' + @tse_sep + DiagnosisCode1 + 
(case when DiagnosisCode2 is not null then @te_sep + 'BF' + @tse_sep + DiagnosisCode2 + 
(case when DiagnosisCode3 is not null then @te_sep + 'BF' + @tse_sep + DiagnosisCode3 + 
(case when DiagnosisCode4 is not null then @te_sep + 'BF' + @tse_sep + DiagnosisCode4 +
(case when DiagnosisCode5 is not null then @te_sep + 'BF' + @tse_sep + DiagnosisCode5 +
(case when DiagnosisCode6 is not null then @te_sep + 'BF' + @tse_sep + DiagnosisCode6 +
(case when DiagnosisCode7 is not null then @te_sep + 'BF' + @tse_sep + DiagnosisCode7 +
(case when DiagnosisCode8 is not null then @te_sep + 'BF' + @tse_sep + DiagnosisCode8 
else rtrim('') end)
else rtrim('') end)
else rtrim('') end)
else rtrim('') end)
...
ReferringEntityQualifier + @te_sep + ReferringLastName + @te_sep + ReferringFirstName 
+ (case when ReferringIdQualifier = 'XX' then 
@te_sep + @te_sep + @te_sep + @te_sep + ReferringIdQualifier + @te_sep + ReferringId 
else rtrim('') end)
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
ReferringRefSegment = NULL,--case when ReferringSecondaryId is null then null else 
--UPPER(replace(replace(replace(replace(replace(replace((
--'REF' + @te_sep + ReferringSecondaryQualifier + @te_sep + ReferringSecondaryId
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
ReferringRef2Segment = NULL,--case when ReferringSecondaryId2 is null then null else 
--UPPER(replace(replace(replace(replace(replace(replace((
--'REF' + @te_sep + ReferringSecondaryQualifier2 + @te_sep + ReferringSecondaryId2
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
ReferringRef3Segment = NULL,--case when ReferringSecondaryId3 is null then null else 
--UPPER(replace(replace(replace(replace(replace(replace((
--'REF' + @te_sep + ReferringSecondaryQualifier3 + @te_sep + ReferringSecondaryId3
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
RenderingNM1Segment = case when isnull(rtrim(RenderingId),'') = '' then null else 
UPPER(replace(replace(replace(replace(replace(replace((
/*NM100*/'NM1' + @te_sep + 
/*NM101*/RenderingEntityCode + @te_sep + --'82'
/*NM102*/RenderingEntityQualifier + @te_sep + 
/*NM103*/RenderingLastName + @te_sep + 
/*NM104*/RenderingFirstName + @te_sep + 
/*NM105*/@te_sep + 
/*NM106*/@te_sep + 
/*NM107*/@te_sep + 
/*NM108*/RenderingIdQualifier + @te_sep + --'XX'
/*NM109*/RenderingId 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
...
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
RenderingRef3Segment = NULL, --case when isnull(rtrim(RenderingSecondaryId3),'') = '' then null else 
--UPPER(replace(replace(replace(replace(replace(replace((
--'REF' + @te_sep + RenderingSecondaryQualifier3 + @te_sep + RenderingSecondaryId3
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
FacilityNM1Segment = NULL,--case when isnull(rtrim(FacilityAddress1),'') = '' 
...
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
FacilityN3Segment = NULL,--case when isnull(rtrim(FacilityAddress1),'') = '' 
--or isnull(rtrim(FacilityCity),'') = '' 
--or isnull(rtrim(FacilityState),'') = '' 
--or isnull(rtrim(FacilityZip),'') = '' then null else 
--UPPER(replace(replace(replace(replace(replace(replace((
...
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
FacilityN4Segment = NULL,--case when isnull(rtrim(FacilityAddress1),'') = '' 
--or isnull(rtrim(FacilityCity),'') = '' 
--or isnull(rtrim(FacilityState),'') = '' 
--or isnull(rtrim(FacilityZip),'') = ''  then null else 
--UPPER(replace(replace(replace(replace(replace(replace((
...
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end, 
FacilityRefSegment = NULL,--case when isnull(rtrim(FacilitySecondaryId),'') = '' then null else 
--UPPER(replace(replace(replace(replace(replace(replace((
--'REF' + @te_sep + FacilitySecondaryQualifier   + @te_sep + 
--FacilitySecondaryId  
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
FacilityRef2Segment = NULL,--case when isnull(rtrim(FacilitySecondaryId2),'') = '' then null else 
--UPPER(replace(replace(replace(replace(replace(replace((
--'REF' + @te_sep + FacilitySecondaryQualifier2   + @te_sep + 
--FacilitySecondaryId2  
--), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
FacilityRef3Segment = NULL--case when isnull(rtrim(FacilitySecondaryId3),'') = '' then null else 
----UPPER(replace(replace(replace(replace(replace(replace((
...
set SubscriberSegment = UPPER(replace(replace(replace(replace(replace(replace((
'SBR' + @te_sep +   PayerSequenceNumber + @te_sep +   SubscriberRelationshipCode
 + 
-- srf 3/22/2011 -- set to group number based on version 5 guide
 case when isnull(rtrim(GroupNumber),'') = '' then @te_sep 
else @te_sep + rtrim(GroupNumber) end +
case when isnull(rtrim(GroupName),'') = '' or isnull(rtrim(GroupNumber),'') <> '' then @te_sep 
else @te_sep + rtrim(GroupName) end
...
@te_sep +  InsuranceTypeCode + @te_sep +  
 @te_sep  + @te_sep  + @te_sep +  ClaimFilingIndicatorCode 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term, 
PayerPaidAmountSegment = UPPER(replace(replace(replace(replace(replace(replace((
'AMT' + @te_sep +  'D' + @te_sep +  convert(varchar,PayerPaidAmount) 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term, 
...
CAS3Segment = case when e.HIPAACode1 is null or e.Amount1 in (0, null) then null else 
UPPER(replace(replace(replace(replace(replace(replace((
'CAS' + @te_sep + 'OA' 
...
'SV1' + @te_sep +  ServiceIdQualifier + @tse_sep + 
rtrim(BillingCode) + 
	Coalesce(@tse_sep + rtrim(Modifier1),'') + 
	Coalesce(@tse_sep + rtrim(Modifier2),'') + 
	Coalesce(@tse_sep + rtrim(Modifier3),'') + 
	Coalesce(@tse_sep + rtrim(Modifier4),'') + @te_sep + 
(case when convert(int,LineItemChargeAmount*100) = convert(int,LineItemChargeAmount)*100 then 
convert(varchar,convert(int,LineItemChargeAmount)) else
convert(varchar,LineItemChargeAmount) end) + @te_sep +  
UnitOfMeasure + @te_sep +  ServiceUnitCount + @te_sep +  PlaceOfService + @te_sep +  
@te_sep +  DiagnosisCodePointer1 +
(case when DiagnosisCodePointer2 is not null then  @tse_sep + DiagnosisCodePointer2 else rtrim('') end) +
(case when DiagnosisCodePointer3 is not null then  @tse_sep + DiagnosisCodePointer3 else rtrim('') end) +
(case when DiagnosisCodePointer4 is not null then  @tse_sep + DiagnosisCodePointer4 else rtrim('') end) + 
(case when DiagnosisCodePointer5 is not null then  @tse_sep + DiagnosisCodePointer5 else rtrim('') end) + 
(case when DiagnosisCodePointer6 is not null then  @tse_sep + DiagnosisCodePointer6 else rtrim('') end) + 
(case when DiagnosisCodePointer7 is not null then  @tse_sep + DiagnosisCodePointer7 else rtrim('') end) + 
(case when DiagnosisCodePointer8 is not null then  @tse_sep + DiagnosisCodePointer8 else rtrim('') end) + 
(case when EmergencyIndicator is not null then @te_sep +  EmergencyIndicator else rtrim('') end)
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
ServiceDateDTPSegment = UPPER(replace(replace(replace(replace(replace(replace((
'DTP' + @te_sep + '472' + @te_sep + ServiceDateQualifier
 + @te_sep + ServiceDate 
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
LineItemControlRefSegment = UPPER(replace(replace(replace(replace(replace(replace((
'REF' + @te_sep + '6R' + @te_sep + LineItemControlNumber
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term,
...
'LIN' + @te_sep + @te_sep +  isnull(rtrim(NationalDrugCodeQualifier),rtrim(''))  + @te_sep + isnull(rtrim(NationalDrugCode),rtrim(''))  
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end,
CTPSegment = case when DrugCodeUnitCount is null then null else UPPER(replace(replace(replace(replace(replace(replace((
'CTP' + @te_sep + @te_sep + @te_sep+  isnull(rtrim(DrugUnitPrice),rtrim(''))  + @te_sep + isnull(rtrim(DrugCodeUnitCount),rtrim(''))  + @te_sep + isnull(rtrim(DrugUnitOfMeasure),rtrim(''))  
), @e_sep,''),@se_sep,''),@seg_term,''),'&',''),@te_sep,@e_sep),@tse_sep,@se_sep)) + @seg_term end
...
@TransactionTrailer = ('SE' + @e_sep + convert(varchar,@NumberOfSegments) + @e_sep +
TransactionSetControlNumberHeader + @seg_term)
...
BillingProviderN4Segment , BillingProviderRefSegment, BillingProviderRef2Segment, BillingProviderRef3Segment, 
...
select @seg17 = 'HL' + @e_sep +  convert(varchar, @HierId) +
	@e_sep + convert(varchar, @ProviderHierId) + @e_sep + 
	'22' + @e_sep + (case when @seg12 is null then '0' else '1' end) + @seg_term
...
OIN4Segment, OIRefSegment, PayerNM1Segment, PayerPaymentDTPSegment,
...
@seg5, @seg6, @seg7, @seg8, @seg9, @seg10, @seg11, @seg12, @seg13
...
update a set 
	ElectronicClaimsData = CONVERT(text,REPLACE(CONVERT(varchar(max),b.DataText),'~',@seg_term)),  
	Status = 4523, 
