...
function HandleUseQuickTxPlan() {
    try {
        ...
        if ($("input[type='radio']:checked").length > 0) {
            ...
            var txtAreaId = $("input[type='radio']:checked").attr('TextAreaId');
            if (CountSelectedCheckBox.length == 1) {
                ...
                var keyTxValue = $("input[type='radio']:checked").attr('keyValue');
    ...
    try {
		...
		if ($('#divPreviousDiagnosis').length > 0 && ShowInViewMode != "true") {
                if (objectPageResponse.ObjectDocumentInformation != null && objectPageResponse.ObjectDocumentInformation.DocumentMode != 'view')		
                    //OpenPage(5763, 10915, 'CustomAjaxRequestType=GetCurrentDiagnosis', null, GetRelativePath(), null, null, pageActionEnum.CustomAjaxCall, null, null, null);
                    GetCurrentDiagnosis();
        }
    }
    catch (err) {
        LogClientSideException(err, 'InitialTreatmentPlan ');
    }
}
...
function GetCurrentDiagnosis() {
    try {
        $.ajax({
            type: "POST",
            url: GetRelativePath() + "ActivityPages/Client/Detail/HarborTreatmentPlan/TreatmentPlanHarborAjaxScript.aspx?GetCurrentDiagnosis",
            data: 'action=GetCurrentDiagnosis',
        ...
        LogClientSideException(err, 'TreatmentPlan-GetCurrentDiagnosis');
    }
}
