...
--Pulled from St.Jow by Vichee on 02/11/2015 w.r.t task #35 in WMU Enhancements
...
IF Not EXISTS(SELECT 1 FROM Documentcodes WHERE DocumentcodeId=15109)
BEGIN
...
Values(15109,'Revoke Release of Information','Revoke Release of Information',10,'Y','N','N','N',
...
IF Not EXISTS(SELECT 1 FROM Screens WHERE ScreenId=31118)
 BEGIN
	INSERT INTO Screens
	...
	VALUES 
	(31118,'Revoke Release of Information',5763,'/Custom/ROI/WebPages/RevokeReleaseOfInformation.ascx',2,15109,'csp_SCInitRevokeReleaseofInformation','csp_SCPostSignatureUpdateRevokeReleaseofInformation',@FormId)
END
else begin
update screens set CustomFieldFormId=@FormId where screenid=31118
end
SET IDENTITY_INSERT Screens off


--Banners Table Entry
IF Not EXISTS(SELECT 1 FROM Banners WHERE ScreenId=31118)
BEGIN
Insert INTO Banners(BannerName,DisplayAs,Active,DefaultOrder,Custom,TabId,ScreenId,ParentBannerId)
Values('Revoke Release of Information','Revoke Release of Information','Y',1,'N',2,31118,21)
set @bannerid=@@IDENTITY
END
ELSE
begin
select @bannerid=bannerid FROM Banners WHERE ScreenId=31118
end

if not exists (select 1 from [DocumentNavigations] where screenid=31118)
begin
INSERT INTO [DocumentNavigations]([NavigationName],[DisplayAs],[Active]
           ,[ParentDocumentNavigationId],[BannerId],[ScreenId])
     VALUES('Revoke Release of Information','Revoke Release of Information','Y',null,@bannerid,31118)
 end    
