IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_RdlSubReportTransferDocumentHeader]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_RdlSubReportTransferDocumentHeader]
GO
...
GO              
CREATE PROCEDURE [dbo].[csp_RdlSubReportTransferDocumentHeader]          
@DocumentVersionId int                      
                              
 AS                               
/*********************************************************************/                                                                                                  
 /* Stored Procedure: [csp_RdlSubReportTransferDocumentHeader]               */                                                                                         
...
BEGIN CATCH                                
 ...
 set @Error= Convert(varchar,ERROR_NUMBER()) + '*****' + Convert(varchar(4000),ERROR_MESSAGE())                             
    + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'csp_RdlSubReportTransferDocumentHeader')                                 
