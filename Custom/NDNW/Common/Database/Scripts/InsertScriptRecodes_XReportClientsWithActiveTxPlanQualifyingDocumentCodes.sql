...
Script:		InsertScriptRecodes_XReportClientsWithActiveTxPlanQualifyingDocumentCodes.sql
...
--############################################################################# 
 ...
 DECLARE @Recode_MappingEntity		Varchar	(max)	-- Value to store in Recodes.MappingEntity /* GlobalCodes */
	...
	SET @Recode_CategoryCode			=  'XReportClientsWithActiveTxPlanQualifyingDocumentCodes'
 	SET @Recode_CategoryName			=  'XReportClientsWithActiveTxPlanQualifyingDocumentCodes'
 	SET @Recode_Description			= 'Document Code IDs that qualify as a Treatment Plan'
 	SET @Recode_MappingEntity			= 'DocumenCodes' --eg. Globalcodes, HealthDataAttributes, Programs
   ...
   INSERT INTO @Codes
     ...
     SELECT DocumentName, NULL, DocumentCodeId FROM DocumentCodes WHERE DocumentCodeId IN (
     1620 -- Service Plan
     ,1632 -- Service Plan Review
     )
