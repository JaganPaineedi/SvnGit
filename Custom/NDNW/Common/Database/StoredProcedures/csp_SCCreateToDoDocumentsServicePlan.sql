/****** Object:  StoredProcedure [dbo].[csp_SCCreateToDoDocumentsServicePlan]    Script Date: 6/27/2018 9:57:38 AM ******/
if object_id('dbo.csp_SCCreateToDoDocumentsServicePlan') is not null 
DROP PROCEDURE [dbo].[csp_SCCreateToDoDocumentsServicePlan]
GO
...
CREATE PROCEDURE [dbo].[csp_SCCreateToDoDocumentsServicePlan]
...
begin
 ...
 DECLARE @AlertType INT
	...
	from DocumentCodes
	where DocumentName = 'SERVICE PLAN' and isnull(RecordDeleted, 'N') = 'N' ), 0)
         ...
         (ToStaffId,
          ...
          Message
                ...
                SELECT D.AuthorId, --h.AlertToUser, --to Staff (service clinician)  
                       ...
                       'Y',
                       GETDATE(),
                       'Care Plan due in 14 Days', --Subject  
                       'Client, '+C.lastname+', '+C.Firstname+' Care Plan is due on '+CONVERT( VARCHAR(20), DATEADD(yyyy, 1, D.EffectiveDate), 101)+'.'
                FROM @NewDocuments N
