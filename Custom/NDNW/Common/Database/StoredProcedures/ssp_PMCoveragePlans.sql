IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ssp_PMCoveragePlans]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ssp_PMCoveragePlans]
GO

/****** Object:  StoredProcedure [dbo].[ssp_PMCoveragePlans]    Script Date: 10/22/2015 12:55:58 PM ******/
SET ANSI_NULLS ON
GO
...
Create PROCEDURE [dbo].[ssp_PMCoveragePlans]  
...
-- 11-June-2014 Gautam          Added code Max(RaceId) reason: CurrentClients count showing wrong due to multiple race related to client, 
								Task#1502, core bugs 
-- !0-Oct-2015	Abhijeet		Modified the Proc to match it with no. of Eligible clients
*******************************************************************************/  
   ...
   --         )  
		...
		FROM CoveragePlans CP              
			...
			LEFT JOIN 
				...
				--Modified to Match eligible clients
				COUNT(Distinct C.clientid) AS CurrentClients
				  ...
				  JOIN ClientCoverageHistory cch ON ccp.ClientCoveragePlanId = cch.ClientCoveragePlanId
					...
					--Removed to match eligible clients
			...
			AND ((@CustomFiltersApplied = 'Y' AND EXISTS(SELECT * FROM #CustomFilters cf WHERE cf.CoveragePlanId = CP.CoveragePlanId)) OR
				...
				AND(@Information = -1 OR													--   Complete and Incomplete Information 
				   ...
				   (@Information = 2 AND ISNULL(CP.InformationComplete,'N') = 'N'))		--   Incomplete Information
				     AND (@CoveragePlanName='' OR (CP.CoveragePlanName LIKE '%' + @CoveragePlanName + '%'))
				)
			)	
