   /****** Object:  StoredProcedure [dbo].[csp_ReportDocumentationDueDates]        ******/
...
vsinha  04/20/2017  Corrected the DocumentCodeId for Assessment and CarePlan for task- New Directions - Support Go Live #629 
*/ 
/********************************************************************/    
/* 
...
and (d.documentcodeid = 101     
  or    
 (d.documentcodeid in (349,1469,10018) and exists (select 1 from CustomHRMAssessments a WITH(NOLOCK)    
...
From #DueDates dd    
Join Documents d WITH(NOLOCK) on dd.ClientId = d.ClientID     
 and d.DocumentCodeId in (2, 350,1620)    
...
left Join TpGeneral tpg WITH(NOLOCK) on tpg.DocumentVersionId = d.CurrentDocumentVersionId and tpg.PlanOrAddendum = 'T'    
 and isnull(tpg.RecordDeleted, 'N') = 'N'    
...
BEGIN CATCH                                                                                           
        ...
        set @Error = CONVERT(varchar, ERROR_NUMBER()) + '*****'  
            ...
            + ISNULL(CONVERT(varchar, ERROR_PROCEDURE()),  
                     'csp_ReportDocumentationDueDates') + '*****'  
