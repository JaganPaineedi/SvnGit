IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[csp_SubReportIncidentManagerReviews]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[csp_SubReportIncidentManagerReviews] 
GO

CREATE PROCEDURE [dbo].[csp_SubReportIncidentManagerReviews] (@IncidentReportManagerReviewId INT)
AS
/********************************************************************************                                                       
--      
...
*********************************************************************************/
BEGIN TRY
	SELECT C.IncidentReportManagerFollowUpId
		,C.CreatedBy
		,C.CreatedDate
		,C.ModifiedBy
		,C.ModifiedDate
	...
	FROM CustomIncidentReportManagerFollowUps C
	...
	WHERE ISNull(C.RecordDeleted, 'N') = 'N'
		AND C.IncidentReportManagerFollowUpId = @IncidentReportManagerReviewId
END TRY

BEGIN CATCH
	DECLARE @Error VARCHAR(8000)

	SET @Error = Convert(VARCHAR, ERROR_NUMBER()) + '*****' + Convert(VARCHAR(4000), ERROR_MESSAGE()) + '*****' + isnull(Convert(VARCHAR, ERROR_PROCEDURE()), 'csp_SubReportIncidentManagerReviews') + '*****' + Convert(VARCHAR, ERROR_LINE()) + '*****' + Convert(VARCHAR, ERROR_SEVERITY()) + '*****' + Convert(VARCHAR, ERROR_STATE())
