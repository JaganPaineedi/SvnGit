IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'csp_RDLSubReportCSSRSAdultLT')
	BEGIN
		DROP  Procedure  csp_RDLSubReportCSSRSAdultLT
	END

GO
Create Procedure [dbo].[csp_RDLSubReportCSSRSAdultLT]
 @DocumentVersionId INT      
AS      
 /*********************************************************************/                                                                                            
 /* Stored Procedure: csp_RDLSubReportCSSRSAdultLT      */                                                                                   
...
FROM CustomDocumentAdultLTs WHERE DocumentVersionId = @DocumentVersionId

END TRY                                                                              
BEGIN CATCH                                  
DECLARE @Error varchar(8000)                                                                               
SET @Error= Convert(varchar,ERROR_NUMBER()) + '*****' + Convert(varchar(4000),ERROR_MESSAGE())                                                         
    + '*****' + isnull(Convert(varchar,ERROR_PROCEDURE()),'csp_RDLSubReportCSSRSAdultLT')                                                                                                             
