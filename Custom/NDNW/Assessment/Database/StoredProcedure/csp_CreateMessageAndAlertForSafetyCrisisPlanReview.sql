  /****** Object:  StoredProcedure [dbo].[csp_CreateMessageAndAlertForSafetyCrisisPlanReview]    Script Date: 12/08/2014 12:45:53 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[csp_CreateMessageAndAlertForSafetyCrisisPlanReview]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[csp_CreateMessageAndAlertForSafetyCrisisPlanReview]
GO

/****** Object:  StoredProcedure [dbo].[csp_CreateMessageAndAlertForSafetyCrisisPlanReview]     ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[csp_CreateMessageAndAlertForSafetyCrisisPlanReview] --  11                         
...
--select  top 10 ReviewEveryXDays, SafetyCrisisPlanReviewed,NextSafetyPlanReviewDate, * from CustomSafetyCrisisPlanReviews order by 1 desc--safety  40033  
  ...
  INSERT INTO [Messages] (  
   ...
   ,60  
      ,5854  
      ,@DocumentId  
      ,(SELECT TOP 1 (DocumentName) FROM DocumentCodes WHERE DocumentCodeId = 40038)  
      ,@DocumentId  
  FROM #StaffTable    
