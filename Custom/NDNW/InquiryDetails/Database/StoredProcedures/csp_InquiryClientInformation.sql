...
IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[csp_InquiryClientInformation]')
			AND type IN (
				N'P'
				,N'PC'
				)
		)
	DROP PROCEDURE [dbo].[csp_InquiryClientInformation]
GO
create PROCEDURE [dbo].[csp_InquiryClientInformation]   
 @ClientId INT ,  
 @InquiryId  INT = Null  
AS    
