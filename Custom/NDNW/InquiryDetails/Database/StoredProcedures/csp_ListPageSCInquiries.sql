IF EXISTS (
		SELECT *
		FROM sys.objects
		WHERE object_id = OBJECT_ID(N'[dbo].[csp_ListPageSCInquiries]')
			AND type IN (
				N'P'
				,N'PC'
				)
...
CREATE PROCEDURE [dbo].[csp_ListPageSCInquiries]                         
 ...
 /*  Date                  Author                 Purpose        */                            
/* 9.27.2013			Katta Sharath Kumar      katta sharath kumar with Ref to task #3 in Ionia County CMH - Customizations         */                                 
  -- =============================================                                                                                                          
 @InstanceId int,                                                                                              
 @PageNumber int,                                                                                                  
 @PageSize int,                                                                                                  
        ...
        WITH listInquiries               
             AS (                    
					  ...
					  isnull(S.LastName,'') + isnull(S.FirstName,'')      
					   ...
					   LEFT JOIN GlobalCodes GC ON GC.GlobalCodeId in (Select CDSTP.Disposition From [CustomDispositions] CDSTP Where CDSTP.InquiryId=CI.InquiryId and ISNULL(CDSTP.RecordDeleted,'N')='N' )        
